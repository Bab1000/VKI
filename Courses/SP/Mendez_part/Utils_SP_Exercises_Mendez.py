...
def PlotExercise1(freq,fft,path):
    ...
    save_path_fft = os.path.join(path, "Results_exercise_1", "fft.pdf")
    ...
    plt.savefig(save_path_fft, format='pdf', dpi=300, bbox_inches='tight')
    plt.close()
   
    print(Fore.GREEN + f"The results have been successfully saved at: {save_path_fft}")
...
def GifGenerator(path,freq_splitting_vector,fs,n_samples):
    ...
    Images_list=[]
    for i in tqdm(np.arange(0, n_samples, 200), desc="--> Generating images for animation of scale 1", unit="img", ncols=120):
        ...
        plt.contourf(Xg, Yg, mode_data, levels=np.linspace(vmin, vmax, 500), cmap="jet")
    ...
    Images_list=[]
    for i in tqdm(np.arange(0, n_samples, 200), desc="--> Generating images for animation of scale 2", unit="img", ncols=120):
    ...
    Images_list=[]
    for i in tqdm(np.arange(0, n_samples, 200), desc="--> Generating images for animation of scale 3", unit="img", ncols=120):
