...
def grad_rosenbrock(w):
    dJdw0 = -2*(1 - w[0]) - 400*w[0]*(w[1] - w[0]**2)
    dJdw1 = 200*(w[1] - w[0]**2)
...
def conj_grad_method(n_iter,w_0,c1,c2,eps,cost,grad,args=()):
    ...
    for i in range(int(n_iter-1)):
        ...
        # Reset the algorithm (to generate new conjugate directions)
        if i % len(w_0) == 0: d[i,:] = -Grad_J[i,:]
...
def BFGS(n_iter,w_0,c1,c2,eps,cost,grad,args=()):
    ...
    B = np.eye(len(w_0)); I = np.eye(len(w_0))
    # Initialize search direction history
    d = np.zeros((int(n_iter),len(w_0))); d[0,:] = -np.dot(B,Grad_J[0,:].T)
    ...
    for i in range(int(n_iter-1)):
        ...
        B_aux_1 = I - rho_k*np.dot(s_k,y_k.T)
        B_aux_2 = I - rho_k*np.dot(y_k,s_k.T)
        B_aux_3 = rho_k*np.dot(s_k,s_k.T)
        B = np.dot(np.dot(B_aux_1,B),B_aux_2) + B_aux_3
        
        # Compute new search direction
        d[i+1,:] = - np.dot(B,Grad_J[i+1,:].T)
