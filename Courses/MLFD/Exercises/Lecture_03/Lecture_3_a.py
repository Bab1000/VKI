...
from Lecture_3_Functions import cost_contour
from Lecture_3_Functions import cost_rosenbrock, grad_rosenbrock
from Lecture_3_Functions import grad_descent, line_search_grad_descent, conj_grad_method, BFGS
...
#%% Run vanilla GD (less iterations) + line-search GD
...
plt.plot(w_3[:,0],w_3[:,1],color='C2',linestyle='solid',linewidth=3,alpha=0.8,marker='',zorder=7)
plt.scatter(w_3[-1,0],w_3[-1,1],color='C2',marker='s',label=r'Conjugate gradients method',zorder=7)
...
plt.xlabel(r'$w_0$'); plt.ylabel(r'$w_1$')
plt.legend(); plt.axis('scaled'); plt.tight_layout()
plt.savefig('myplot.png',dpi=350)
...
plt.plot(w_4[:,0],w_4[:,1],color='C3',linestyle='solid',linewidth=3,alpha=0.8,marker='',zorder=8)
