...
fn.visualize(t, data_plot, save='trajectory '+dataSet,fz=10)
...
model = LSTM(input_size=3, hidden_dim=128, output_size=3, n_layers=2)
...
for epoch in range(epochs):
    ...
    if epoch%50 == 0:
        ...
        fn.visualize(t,data_plot,prediction,iter=epoch,save='plots/LSTM '+dataSet+ f' prediction epoch {epoch}')
...
plt.plot(np.arange(epochs,step=50),losses_val,label='validation loss')
plt.yscale('log')
plt.grid()
plt.xlabel('epochs')
plt.ylabel('error')
...
fn.make_gif(name='LSTM '+dataSet)

#%% Assess forecast capability
if dataSize<250:
...
with torch.no_grad():
    for i in range(len(t)):
        ...
        prediction.append(model_out)
prediction = np.array(prediction).T
fn.visualize(t,data_plot,prediction[:,batchLength:],iter= 'Forecast',save='Forecast LSTM '+dataSet)
