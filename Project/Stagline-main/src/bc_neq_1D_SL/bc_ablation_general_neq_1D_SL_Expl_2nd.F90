...
!! nonequilibrium flows. 
        ...
        SUBROUTINE no_slip_isothermal_general_ablation_neq_1D_SL_Expl_2nd (id, phys_data1, u_phys1, phys_data2, u_phys2,   & 
        ...
        REAL(KIND=8),  DIMENSION(nb_temp) :: Twall, T2,Tghost
...
! Pass mole fractions of first physical cell and distance from the wall (half volume)
       ...
       ENDDO
        ...
        prim(pos_u) =  MAX(3*uwall - 2*phys_data2(pos_u_cell),0.9d0*uwall)
        prim(pos_v) = -phys_data2(pos_v_cell)

        ! Compute the ghost state conservative variables and physical properties from primitive variables 
        CALL get_cons_phys_from_prim(prim, u_ghost2, ghost_data2)

        END SUBROUTINE no_slip_isothermal_general_ablation_neq_1D_SL_Expl_2nd
!------------------------------------------------------------------------------------------------------------------------------!
!------------------------------------------------------------------------------------------------------------------------------!
