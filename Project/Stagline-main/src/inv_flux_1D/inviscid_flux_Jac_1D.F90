!------------------------------------------------------------------------------!
!> This subroutine computes the inviscid flux Jacobian for the 1D Euler equations for a calorically perfect gas
  SUBROUTINE inviscid_flux_Jac_1D (nx, phys_data, a) 

    USE mod_general_data,         ONLY: pos_u_cell, pos_h0_cell, pos_c_cell, pos_ek_cell, gamma

    IMPLICIT NONE

    REAL(KIND=8) :: gamma_minus1, gamma_plus1, gamma_minus3, gamma_minus1_v2
    REAL(KIND=8) :: c, ek, h0, u, vn
    ...
    ! Velocity, total enthalpy and speed of sound
    ...
    a(3,1) = vn*(0.5d0*gamma_minus1_v2 - h0)

    ! Second column
    a(1,2) = nx
    a(2,2) = - gamma_minus3*vn
    a(3,2) = (h0 - gamma_minus1_v2)*nx
        
    ! Third column
    a(1,3) = 0.d0
    a(2,3) = gamma_minus1*nx
    a(3,3) = gamma*vn 
