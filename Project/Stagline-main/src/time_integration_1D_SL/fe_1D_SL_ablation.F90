!------------------------------------------------------------------------------!
!> This subroutine computes the solution variation in case of use of an explicit time-integration scheme scheme for 1D
!! stagnation line flows when an ablative wall is investigated.
!! The subroutine is identical to the original "fe_1D_SL" except that for the
...
#include"../config.h"
    ...
    USE mod_general_data,                ONLY: nb_cells, nb_tot_cells, nb_eq, nb_prop, start_u_phys, start_prop_phys,     & 
                                             & volumes, s_vec, xc, u, du, cell_prop,                                      &
    ...
    USE mod_function_pointer,            ONLY: conv_flux_1D_SL, diff_flux_1D_SL, compute_time_step_1D_SL, rec_1D_SL,      & 
                                             & get_inv_source_term_SL, get_diff_source_term_SL
    USE mod_neq_function_pointer,        ONLY: library_get_species_enthalpies, library_compute_eq_composition_pyro
    USE mod_ablation_data,               ONLY: omega_wall, mdot_wall_tot, tau_rr_wall, tau_rt_wall, conv_flux_wall,        &
                                             & uwall, Twall, phi_pyro
    ...
    DO i = 1,nb_cells
       ...
       ! Inviscid source term(s)
       s = 0.d0
       DO j = 1,nb_inv_source_terms
          CALL get_inv_source_term_SL(j)%source(i, rc_l, prop_l, ul, s_vec)
          s = s + s_vec
       ENDDO 

       ! Diffusive source term
       CALL get_diff_source_term_SL(rc_l, vol_ll, vol_l, vol_r, prop_ll, prop_l, prop_r, ull, ul, ur, s_vec) 
       s = s + s_vec
  ...
  END SUBROUTINE fe_1D_SL_ablation
!------------------------------------------------------------------------------!
