!------------------------------------------------------------------------------!
! This module implements an interface for the Nitrogen_FHO thermodynamic library. It provides an
...
! Therefore it can plugged to any code without big efforts. 
  MODULE mod_Nitrogen_FHO_library 
...
#include"../config.h"
    ...
    INTEGER, SAVE :: nb_ns, nb_bins, nb_tvib, nb_te, nb_trot, nb_temp, nb_eq, nb_dim 
    ...
    ! Subroutines of Nitrogen_FHO thermodynamic library interface. In some cases they call 
    ...
    CONTAINS
      ...
      SUBROUTINE Nitrogen_FHO_initialize (input_ns, input_ntrot, input_ntvib, input_nte, input_ntemp, input_neq,  & 
                                        & input_ndim, input_solver, input_mixture, input_reaction, state_model,   &
                                        & input_transf, input_path, input_xi_tol) 
 
        USE mod_nitrogen_FHO_initialize_CFD,            ONLY: initialize, EkJ
        ...
        IF (nt.NE.nb_temp) THEN
           PRINT*
           WRITE(*,5)input_solver(1:length),'::Nitrogen_FHO_library interface'   
        ...
        ! Nitrogen_FHO library is initialized
...
5     FORMAT(A,A)
      ...
      END SUBROUTINE Nitrogen_FHO_initialize
 
      !------------------------------------------------------!
      ! This subroutine finalizes the Nitrogen FHO thermodynamic library
      SUBROUTINE Nitrogen_FHO_finalize

         USE mod_nitrogen_FHO_initialize_CFD,            ONLY: finalize
      ...
      END SUBROUTINE Nitrogen_FHO_finalize

      !------------------------------------------------------!
      ! This subroutine computes post-shock nonequilibrium conditions
      SUBROUTINE Nitrogen_FHO_post_shock_neq (p1, u1, T1, p2, u2, T2, yi, xN)

        USE mod_nitrogen_FHO_CFD_prop,                  ONLY: post_shock_neq
      ...
      END SUBROUTINE Nitrogen_FHO_post_shock_neq

      !------------------------------------------------------!
      ! This subroutine computes post-shock equilibrium conditions
      SUBROUTINE Nitrogen_FHO_post_shock_eq (p1, u1, T1, p2, u2, T2, yi, rhoi, xN)

        USE mod_nitrogen_FHO_CFD_eq,                  ONLY: post_shock_eq
      ...
      END SUBROUTINE Nitrogen_FHO_post_shock_eq
      ...
      SUBROUTINE Nitrogen_FHO_write_ODE_solution (flow_file, pop_file, k, xold, x, uold, m_dot, z)

        INTEGER :: is
        REAL(KIND=8) :: mm, fac, facN
        REAL(KIND=8) :: u, p, rho
...
20    FORMAT(A,I6,100E20.10)
      ...
      END SUBROUTINE Nitrogen_FHO_write_ODE_solution 

      !------------------------------------------------------!
      ! This subroutine post-process the ODE solution for the flow 
      ! behind a normal shock wave
      SUBROUTINE Nitrogen_FHO_post_process_ODE_solution (post_file, x, ni, temp) 

        USE mod_nitrogen_FHO_CFD_prop,            ONLY: compute_Tint
...
10    FORMAT(100E20.10)
      ...
      END SUBROUTINE Nitrogen_FHO_post_process_ODE_solution 

      !------------------------------------------------------!
      ! This subroutine writes down the solution when solving the flow equations 
      ! by means of Finite Volume method
      SUBROUTINE Nitrogen_FHO_write_fvmcc_solution (flow_file, pop_file, k, xold, x, uold, u, rhoi, temp)

        INTEGER :: is
        REAL(KIND=8) :: mm, fac
        REAL(KIND=8) :: p, rho
        REAL(KIND=8), SAVE :: time, T, Tint
        REAL(KIND=8), DIMENSION(2) :: xi
        ...
        ! Density 
        CALL Nitrogen_FHO_get_density (rhoi, rho)

        ! Species mass fractions
        CALL Nitrogen_FHO_get_mass_fractions (rho, rhoi, yi)
        ...
        T = temp(1)
        CALL Nitrogen_FHO_compute_Tint (ni, T, Tint)
...
20    FORMAT(A,I6,100E20.10)
      ...
      END SUBROUTINE Nitrogen_FHO_write_fvmcc_solution  

      !------------------------------------------------------!
      ! This subroutine computes the equilibrium composition given pressure and temperature
      SUBROUTINE Nitrogen_FHO_compute_eq_composition (p, T, rhoi)
      ...
      END SUBROUTINE Nitrogen_FHO_compute_eq_composition

      !------------------------------------------------------!
      ! This subroutine computes the macroscopic equilibrium composition
      SUBROUTINE Nitrogen_FHO_compute_eq_composition_macro (p, T, rhoi)

        USE mod_nitrogen_FHO_CFD_eq,           ONLY: eq_composition
      ...
      END SUBROUTINE Nitrogen_FHO_compute_eq_composition_macro 

      !------------------------------------------------------!
      ! This subroutine computes the N-N2 equilibrium internal energy per unit mass 
      SUBROUTINE Nitrogen_FHO_compute_eq_species_energy (T, e)

        USE mod_nitrogen_FHO_CFD_eq,           ONLY: energy
      ...
      END SUBROUTINE Nitrogen_FHO_compute_eq_species_energy 

      !------------------------------------------------------!
      ! This subroutine computes the N-N2 equilibrium enthalpy per unit mass 
      SUBROUTINE Nitrogen_FHO_compute_eq_species_enthalpy (T, h)

        USE mod_nitrogen_FHO_CFD_eq,           ONLY: enthalpy
      ...
      END SUBROUTINE Nitrogen_FHO_compute_eq_species_enthalpy 
      ...
      SUBROUTINE Nitrogen_FHO_compute_eq_species_entropy (p, T, yi, xi, smix, s)

        USE mod_nitrogen_FHO_CFD_eq,           ONLY: entropy
      ...
      END SUBROUTINE Nitrogen_FHO_compute_eq_species_entropy 

      !------------------------------------------------------!
      ! This subroutine extracts the internal temperature given the energy level population
      SUBROUTINE Nitrogen_FHO_compute_Tint (ni, T, Tint)
      ...
      END SUBROUTINE Nitrogen_FHO_compute_Tint

      !------------------------------------------------------!
      ! This subroutine computes the temperatures and thermodynamic data,
      ! additional data for Jacobians are given in output
      SUBROUTINE Nitrogen_FHO_get_data (rho, rhoi, rho_eint, temp, c, gamma, p, alpha, beta, ei)
  
        USE mod_nitrogen_FHO_CFD_prop,     ONLY: compute_T, get_species_energy_cv
      ...
      END SUBROUTINE Nitrogen_FHO_get_data 
  
      !------------------------------------------------------!
      ! This subroutine computes thermodynamic data needed to be stored
      ! for nonequilibrium gas flow computations. 
      SUBROUTINE Nitrogen_FHO_get_thermodynamic_data (rho, rhoi, temp, c, gamma, p, alpha, beta, ei)
 
        USE mod_nitrogen_FHO_CFD_prop,     ONLY: get_species_cv, get_species_energy
      ...
      END SUBROUTINE Nitrogen_FHO_get_thermodynamic_data
 
      !------------------------------------------------------!
      ! This subroutine computes the energy density for total internal,
      ! rotational, vibrational and electronic components.
      SUBROUTINE Nitrogen_FHO_get_energy_densities (rhoi, temp, rho_e)
  
        USE mod_nitrogen_FHO_CFD_prop,     ONLY:  get_species_energy
      ...
      END SUBROUTINE Nitrogen_FHO_get_energy
  
      !------------------------------------------------------!
      ! This subroutine computes the total internal, rotational, vibrational and electronic enthalpy 
      ! per unit mass of the mixture.
      SUBROUTINE Nitrogen_FHO_get_enthalpy (rho, rhoi, temp, h)
        ...
        CALL Nitrogen_FHO_get_species_enthalpy (temp, hint)
      ...
      END SUBROUTINE Nitrogen_FHO_get_enthalpy 
  
      !------------------------------------------------------!
      ! This subroutine computes the species internal energy per unit mass  
      SUBROUTINE Nitrogen_FHO_get_species_energy (temp, e)
      ...
      END SUBROUTINE Nitrogen_FHO_get_species_energy

      !------------------------------------------------------!
      ! This subroutine computes the species enthalpy per unit mass 
      SUBROUTINE Nitrogen_FHO_get_species_enthalpy (temp, h)
      ...
      END SUBROUTINE Nitrogen_FHO_get_species_enthalpy
  
      !------------------------------------------------------!
      ! This subroutine computes the species internal energy and constant 
      ! volume specific heat per unit mass  
      SUBROUTINE Nitrogen_FHO_get_species_energy_cv (temp, e, cv)
  
        USE mod_Nitrogen_FHO_CFD_prop,     ONLY:  get_species_energy_cv
      ...
      END SUBROUTINE Nitrogen_FHO_get_species_energy_cv

      !------------------------------------------------------!
      ! This subroutine computes the species enthalpy and constant pressure
      ! specific heat per unit mass
      SUBROUTINE Nitrogen_FHO_get_species_enthalpy_cp (temp, h, cp)
      ...
      END SUBROUTINE Nitrogen_FHO_get_species_enthalpy_cp

      !------------------------------------------------------!
      ! This subroutine computes the frozen specific heats at constant volume of each species
      SUBROUTINE Nitrogen_FHO_get_frozen_cv (temp, cv)  
 
        USE mod_Nitrogen_FHO_CFD_prop,     ONLY:  get_species_cv
      ...
      END SUBROUTINE Nitrogen_FHO_get_frozen_cv 
  
      !------------------------------------------------------!
      ! This subroutine computes the frozen specific heats at constant pressure of each species
      SUBROUTINE Nitrogen_FHO_get_frozen_cp (temp, cp)  
  
        USE mod_Nitrogen_FHO_CFD_prop,     ONLY:  get_species_cp
      ...
      END SUBROUTINE Nitrogen_FHO_get_frozen_cp

      !------------------------------------------------------!
      ! This subroutine computes the temperatures given the energy densities.
      SUBROUTINE Nitrogen_FHO_get_temperatures (rhoi, eint, temp)
  
        USE mod_nitrogen_FHO_CFD_prop,    ONLY: compute_T
      ...
      END SUBROUTINE Nitrogen_FHO_get_temperatures
 
      !------------------------------------------------------!
      ! This subroutine computes the equilibrium speed of sound
      SUBROUTINE Nitrogen_FHO_get_eq_sound_speed (rhoi, p, T, c)
        ...
        CALL Nitrogen_FHO_compute_eq_composition_macro (p_pert, T, dens)
        CALL Nitrogen_FHO_get_density (dens, rho_pert)
        ...
        CALL Nitrogen_FHO_compute_eq_composition_macro (p, T_pert, dens)
        CALL Nitrogen_FHO_get_density (dens, rho_pert)
        CALL Nitrogen_FHO_get_mass_fractions (rho_pert, dens, yi)
      ...
      END SUBROUTINE Nitrogen_FHO_get_eq_sound_speed 

      !------------------------------------------------------!
      ! This subroutine computes the equilibrium specific heat ratio
      SUBROUTINE Nitrogen_FHO_get_eq_gamma (rhoi, p, T, gamma)
      ...
      END SUBROUTINE Nitrogen_FHO_get_eq_gamma 

      !------------------------------------------------------!
      ! This subroutine computes the equilibrium specific heat ratio
      SUBROUTINE Nitrogen_FHO_get_eq_gamma_sound_speed (rhoi, p, T, gamma, c)
      ...
      END SUBROUTINE Nitrogen_FHO_get_eq_gamma_sound_speed 
 
      !------------------------------------------------------!
      ! This subroutine computes the frozen speed of sound
      SUBROUTINE Nitrogen_FHO_get_frozen_sound_speed (rhoi, temp, c)
      ...
      END SUBROUTINE Nitrogen_FHO_get_frozen_sound_speed
 
      !------------------------------------------------------!
      ! This subroutine gets the frozen speed specific heat ratio.
      SUBROUTINE Nitrogen_FHO_get_frozen_gamma (rhoi, temp, gamma)
      ...
      END SUBROUTINE Nitrogen_FHO_get_frozen_gamma
 
      !------------------------------------------------------!
      ! This subroutine gets the frozen speed of sound and specific heat ratio.
      SUBROUTINE Nitrogen_FHO_get_frozen_gamma_sound_speed (rhoi, temp, gamma, c)
      ...
      END SUBROUTINE Nitrogen_FHO_get_frozen_gamma_sound_speed
  
      !------------------------------------------------------!
      ! This subroutine computes the mixture static pressure (presence of free electrons is accounted for)
      SUBROUTINE Nitrogen_FHO_get_pressure (rhoi, temp, p)
      ...
      END SUBROUTINE Nitrogen_FHO_get_pressure
  
      !------------------------------------------------------!
      ! This subroutine computes the mixture density.
      SUBROUTINE Nitrogen_FHO_get_density (rhoi, rho)
      ...
      END SUBROUTINE Nitrogen_FHO_get_density
  
      !------------------------------------------------------!
      ! This subroutine computes the specific gas constant R.
      SUBROUTINE Nitrogen_FHO_get_Rgas (rhoi, R)
      ...
      END SUBROUTINE Nitrogen_FHO_get_Rgas
  
      !------------------------------------------------------!
      ! This subroutine gives the single species gas constants
      SUBROUTINE Nitrogen_FHO_get_Ri (out_Ri)
      ...
      END SUBROUTINE Nitrogen_FHO_get_Ri 
  
      !------------------------------------------------------!
      ! This subroutine gives the species molar masses 
      SUBROUTINE Nitrogen_FHO_get_mi (out_mi)
      ...
      END SUBROUTINE Nitrogen_FHO_get_mi

      !------------------------------------------------------!
      ! This subroutine computes the mixture number density 
      SUBROUTINE Nitrogen_FHO_get_mix_numb_dens (p, T, Te, xi, nb) 
      ...
      END SUBROUTINE Nitrogen_FHO_get_mix_numb_dens 

      !------------------------------------------------------!
      ! This subroutine computes the species mass fraction
      SUBROUTINE Nitrogen_FHO_get_mass_fractions (rho, rhoi, yi)
      ...
      END SUBROUTINE Nitrogen_FHO_get_mass_fractions
  
      !------------------------------------------------------!
      ! This subroutine computes the species molar fractions
      SUBROUTINE Nitrogen_FHO_get_molar_fractions (rhoi, xi)
      ...
      END SUBROUTINE Nitrogen_FHO_get_molar_fractions
      ...
      SUBROUTINE Nitrogen_FHO_molar_frac_from_mass_frac (yi, mm, xi)
      ...
      END SUBROUTINE Nitrogen_FHO_molar_frac_from_mass_frac 
     
      !------------------------------------------------------!
      ! This subroutine computes the mass fractions from molar fractions. 
      ! The mixture molar mass is also computed and given in output.
      SUBROUTINE Nitrogen_FHO_mass_frac_from_molar_frac (xi, mm, yi)
      ...
      END SUBROUTINE Nitrogen_FHO_mass_frac_from_molar_frac
      ...
      SUBROUTINE Nitrogen_FHO_comp_tol(xi)
      ...
      END SUBROUTINE Nitrogen_FHO_comp_tol

      !------------------------------------------------------!
      ! This subroutine computes the source term due to collisional and radiative processes.
      SUBROUTINE Nitrogen_FHO_get_source (rhoi, temp, s) 
  
        USE mod_nitrogen_FHO_CFD_source,         ONLY: source
        ...
        T = temp(1)
        CALL source (rhoi, T, s)        
  
      END SUBROUTINE Nitrogen_FHO_get_source 

      !------------------------------------------------------!
      ! This subroutine computes the source term due to collisional and radiative processes 
      ! and its Jacobian (with respect to primitive variables).
      SUBROUTINE Nitrogen_FHO_get_source_Jac (rhoi, temp, s, js)    

        USE mod_nitrogen_FHO_CFD_source,         ONLY: source_Jac
        ...
        T = temp(1)
        CALL source_Jac (rhoi, T, s, js)

      END SUBROUTINE Nitrogen_FHO_get_source_Jac
 
      !------------------------------------------------------!
      ! This subroutine computes the species mass diffusion flux 
      SUBROUTINE Nitrogen_FHO_compute_species_DiffFlux (p, T, Te, xi, diff_driv, Ji) 
        ...
        PRINT*,'in "Nitrogen_FHO_compute_species_DiffFlux", not implemented yet..'
        STOP 

      END SUBROUTINE Nitrogen_FHO_compute_species_DiffFlux
  
      !------------------------------------------------------!
      ! This subrouotine computes the transport coefficients 
      SUBROUTINE Nitrogen_FHO_compute_transpCoeff (p, xi, temp, mu, kappa, lambda, Di, chi) 
        ...
        PRINT*,'in "Nitrogen_FHO_compute_transpCoeff", not implemented yet..'
        STOP 

      END SUBROUTINE Nitrogen_FHO_compute_transpCoeff
#endif 
  
  END MODULE mod_Nitrogen_FHO_library
!------------------------------------------------------------------------------!  
