!------------------------------------------------------------------------------!
! This module implements an interface for the Nitrogen_DSMC thermodynamic library. It provides an
...
! Therefore it can plugged to any code without big efforts. 
  MODULE mod_Nitrogen_DSMC_library 
...
#include"../config.h"
    ...
    ! Subroutines of Nitrogen_DSMC thermodynamic library interface. In some cases they call 
    ...
    CONTAINS
      ...
      SUBROUTINE Nitrogen_DSMC_initialize (input_ns, input_ntrot, input_ntvib, input_nte, input_ntemp, input_neq,  & 
                                         & input_ndim, input_solver, input_mixture, input_reaction, state_model,   &
                                         & input_transf, input_path, input_xi_tol) 
  
        USE mod_nitrogen_DSMC_initialize_CFD,            ONLY: initialize
        USE mod_nitrogen_DSMC_zeta,                      ONLY: set_pointer 
        ...
        IF (nt.NE.nb_temp) THEN
           PRINT*
           WRITE(*,5)input_solver(1:length),'::Nitrogen_DSMC_library interface'   
           PRINT*
        ...
        ! Nitrogen_DSMC library is initialized
...
5     FORMAT(A,A)
      ...
      END SUBROUTINE Nitrogen_DSMC_initialize

      !------------------------------------------------------!
      ! This subroutine finalizes the Nitrogen DSMC thermodynamic library
      SUBROUTINE Nitrogen_DSMC_finalize

         USE mod_nitrogen_DSMC_initialize_CFD,            ONLY: finalize
      ...
      END SUBROUTINE Nitrogen_DSMC_finalize

      !------------------------------------------------------!
      ! This subroutine computes post-shock nonequilibrium conditions
      SUBROUTINE Nitrogen_DSMC_post_shock_neq (p1, u1, T1, p2, u2, T2, yi, xN)

        USE mod_nitrogen_DSMC_CFD_prop,                  ONLY: post_shock_neq
      ...
      END SUBROUTINE Nitrogen_DSMC_post_shock_neq

      !------------------------------------------------------!
      ! This subroutine computes post-shock equilibrium conditions
      SUBROUTINE Nitrogen_DSMC_post_shock_eq (p1, u1, T1, p2, u2, T2, yi, rhoi, xN)

        USE mod_nitrogen_DSMC_CFD_eq,                  ONLY: post_shock_eq
      ...
      END SUBROUTINE Nitrogen_DSMC_post_shock_eq

      !------------------------------------------------------!
      ! This subroutine computes common factors for the ODE system in 
      ! case of thermla nonequilibrium
      SUBROUTINE Nitrogen_DSMC_compute_ODE_factors_tcneq (temp, omega, yi, fac_num, fac_den)

        USE mod_nitrogen_DSMC_initialize_CFD,            ONLY: pos_N2, theta_vib, Rn2
      ...
      END SUBROUTINE Nitrogen_DSMC_compute_ODE_factors_tcneq
      ...
      SUBROUTINE Nitrogen_DSMC_write_ODE_solution (flow_file, pop_file, k, xold, x, uold, m_dot, z)
...
10    FORMAT(100E20.10)
      ...
      END SUBROUTINE Nitrogen_DSMC_write_ODE_solution 

      !------------------------------------------------------!
      ! This subroutine writes down the solution when solving the flow equations 
      ! by means of Finite Volume method
      SUBROUTINE Nitrogen_DSMC_write_fvmcc_solution (flow_file, pop_file, k, xold, x, uold, u, rhoi, temp)
        ...
        ! Density 
        CALL Nitrogen_DSMC_get_density (rhoi, rho)

        ! Species mass fractions
        CALL Nitrogen_DSMC_get_mass_fractions (rho, rhoi, yi)
...
10    FORMAT(100E20.10)
      ...
      END SUBROUTINE Nitrogen_DSMC_write_fvmcc_solution   

      !------------------------------------------------------!
      ! This subroutine computes the equilibrium composition given pressure and temperature
      SUBROUTINE Nitrogen_DSMC_compute_eq_composition (p, T, rhoi)
      ...
      END SUBROUTINE Nitrogen_DSMC_compute_eq_composition

      !------------------------------------------------------!
      ! This subroutine computes the N-N2 equilibrium internal energy per unit mass 
      SUBROUTINE Nitrogen_DSMC_compute_eq_species_energy (T, e)

        USE mod_nitrogen_DSMC_CFD_eq,           ONLY: energy
      ...
      END SUBROUTINE Nitrogen_DSMC_compute_eq_species_energy 

      !------------------------------------------------------!
      ! This subroutine computes the N-N2 equilibrium enthalpy per unit mass 
      SUBROUTINE Nitrogen_DSMC_compute_eq_species_enthalpy (T, h)

        USE mod_nitrogen_DSMC_CFD_eq,           ONLY: enthalpy
      ...
      END SUBROUTINE Nitrogen_DSMC_compute_eq_species_enthalpy 
      ...
      SUBROUTINE Nitrogen_DSMC_compute_eq_species_entropy (p, T, yi, xi, smix, s)

        USE mod_nitrogen_DSMC_CFD_eq,           ONLY: entropy
      ...
      END SUBROUTINE Nitrogen_DSMC_compute_eq_species_entropy   

      !------------------------------------------------------!
      ! This subroutine computes the temperatures and thermodynamic data,
      ! additional data for Jacobians are given in output
      SUBROUTINE Nitrogen_DSMC_get_data (rho, rhoi, rho_eint, temp, c, gamma, p, alpha, beta, ei)
 
        USE mod_nitrogen_DSMC_initialize_CFD,            ONLY: pos_N2 
        USE mod_function_pointer_DSMC,                   ONLY: get_temp_DSMC, get_species_cv_DSMC 
        USE mod_nitrogen_DSMC_CFD_prop,                  ONLY: get_species_energy, ev_cvv_ho 
        ...
        IF (nb_int_temp.GE.1) THEN
           ...
           pos = MAX(2,posTr)
           IF (nb_trot.EQ.1) beta(posTr) = rhoi(pos_N2)*cvr
           ...
           ELSE 
              ...
              ei(nb_ns + nb_int_temp + pos - 1)   = er
              ei(nb_ns + nb_int_temp + posTv - 1) = ev

           ENDIF

        ENDIF
 
      END SUBROUTINE Nitrogen_DSMC_get_data
  
      !------------------------------------------------------!
      ! This subroutine computes thermodynamic data needed to be stored
      ! for nonequilibrium gas flow computations. 
      SUBROUTINE Nitrogen_DSMC_get_thermodynamic_data (rho, rhoi, temp, c, gamma, p, alpha, beta, ei)
      ...
      END SUBROUTINE Nitrogen_DSMC_get_thermodynamic_data
 
      !------------------------------------------------------!
      ! This subroutine computes the energy density for total internal,
      ! rotational, vibrational and electronic components.
      SUBROUTINE Nitrogen_DSMC_get_energy_densities (rhoi, temp, rho_e)
 
        USE mod_nitrogen_DSMC_initialize_CFD,       ONLY: pos_N2 
        USE mod_nitrogen_DSMC_CFD_prop,             ONLY: energy  
      ...
      END SUBROUTINE Nitrogen_DSMC_get_energy
  
      !------------------------------------------------------!
      ! This subroutine computes the total internal, rotational, vibrational and electronic enthalpy 
      ! per unit mass of the mixture.
      SUBROUTINE Nitrogen_DSMC_get_enthalpy (rho, rhoi, temp, h)
  
        USE mod_nitrogen_DSMC_initialize_CFD,      ONLY: pos_N2
        USE mod_nitrogen_DSMC_CFD_prop,            ONLY: enthalpy  
      ...
      END SUBROUTINE Nitrogen_DSMC_get_enthalpy 
 
      !----------------------------------------------------!
      ! This subroutine computes the species internal energies and constant 
      ! volume specific heat per unit mass
      SUBROUTINE Nitrogen_DSMC_get_species_energy (temp, e) 
 
        USE mod_Nitrogen_DSMC_CFD_prop,    ONLY: get_species_energy
      ...
      END SUBROUTINE Nitrogen_DSMC_get_species_energy

      !----------------------------------------------------!
      ! This subroutine computes the species internal energies heat per unit mass
      SUBROUTINE Nitrogen_DSMC_get_species_energy_cv (temp, e, cv) 
 
        USE mod_Nitrogen_DSMC_CFD_prop,    ONLY: get_species_energy_cv
      ...
      END SUBROUTINE Nitrogen_DSMC_get_species_energy_cv

      !----------------------------------------------------!
      ! This subroutine computes the species internal energies and constant 
      ! volume specific heat per unit mass
      SUBROUTINE Nitrogen_DSMC_get_species_enthalpy (temp, h) 
      ...
      END SUBROUTINE Nitrogen_DSMC_get_species_enthalpy

      !----------------------------------------------------!
      ! This subroutine computes the species enthalpies and constant 
      ! pressure specific heat per unit mass
      SUBROUTINE Nitrogen_DSMC_get_species_enthalpy_cp (temp, h, cp) 
      ...
      END SUBROUTINE Nitrogen_DSMC_get_species_enthalpy_cp

      !------------------------------------------------------!
      ! This subroutine computes the frozen specific heats at constant volume of each species
      SUBROUTINE Nitrogen_DSMC_get_species_frozen_cv (T, cv)  
      ...
      END SUBROUTINE Nitrogen_DSMC_get_species_frozen_cv 
  
      !------------------------------------------------------!
      ! This subroutine computes the frozen specific heats at constant pressure of each species
      SUBROUTINE Nitrogen_DSMC_get_species_frozen_cp (T, cp)  
      ...
      END SUBROUTINE Nitrogen_DSMC_get_species_frozen_cp
      ...
      SUBROUTINE Nitrogen_DSMC_get_species_frozen_energy_cv (temp, e, cv)  
 
        USE mod_function_pointer_DSMC,      ONLY: get_species_energy_cv_DSMC   
        ...
        T = temp(1)
        CALL get_species_energy_cv_DSMC (T, cv, e)

      END SUBROUTINE Nitrogen_DSMC_get_species_frozen_energy_cv 
      ...
      SUBROUTINE Nitrogen_DSMC_get_species_frozen_enthalpy_cp (temp, h, cp)  
        ...
        T = temp(1)
        CALL get_species_energy_cv_DSMC (T, cp, h)
      ...
      END SUBROUTINE Nitrogen_DSMC_get_species_frozen_enthalpy_cp

      !------------------------------------------------------!
      ! This subroutine computes the temperatures given the energy densities.
      SUBROUTINE Nitrogen_DSMC_get_temperatures (rhoi, rho_eint, temp)
      ...
      END SUBROUTINE Nitrogen_DSMC_get_temperatures
 
      !------------------------------------------------------!
      ! This subroutine computes the equilibrium speed of sound
      SUBROUTINE Nitrogen_DSMC_get_eq_sound_speed (rhoi, p, T, c)
        ...
        CALL Nitrogen_DSMC_compute_eq_composition (p_pert, T, dens)
        CALL Nitrogen_DSMC_get_density (dens, rho_pert)
        ...
        CALL Nitrogen_DSMC_compute_eq_composition (p, T_pert, dens)
        CALL Nitrogen_DSMC_get_density (dens, rho_pert)
        CALL Nitrogen_DSMC_get_mass_fractions (rho_pert, dens, yi)
      ...
      END SUBROUTINE Nitrogen_DSMC_get_eq_sound_speed 

      !------------------------------------------------------!
      ! This subroutine computes the equilibrium specific heat ratio
      SUBROUTINE Nitrogen_DSMC_get_eq_gamma (rhoi, p, T, gamma)
      ...
      END SUBROUTINE Nitrogen_DSMC_get_eq_gamma 

      !------------------------------------------------------!
      ! This subroutine computes the equilibrium specific heat ratio
      SUBROUTINE Nitrogen_DSMC_get_eq_gamma_sound_speed (rhoi, p, T, gamma, c)
      ...
      END SUBROUTINE Nitrogen_DSMC_get_eq_gamma_sound_speed 
 
      !------------------------------------------------------!
      ! This subroutine computes the frozen speed of sound
      SUBROUTINE Nitrogen_DSMC_get_frozen_sound_speed (rhoi, temp, c)
      ...
      END SUBROUTINE Nitrogen_DSMC_get_frozen_sound_speed
 
      !------------------------------------------------------!
      ! This subroutine gets the frozen speed specific heat ratio.
      SUBROUTINE Nitrogen_DSMC_get_frozen_gamma (rhoi, temp, gamma)
      ...
      END SUBROUTINE Nitrogen_DSMC_get_frozen_gamma
 
      !------------------------------------------------------!
      ! This subroutine gets the frozen speed of sound and specific heat ratio.
      SUBROUTINE Nitrogen_DSMC_get_frozen_gamma_sound_speed (rhoi, temp, gamma, c)
      ...
      END SUBROUTINE Nitrogen_DSMC_get_frozen_gamma_sound_speed
  
      !------------------------------------------------------!
      ! This subroutine computes the mixture static pressure (presence of free electrons is accounted for)
      SUBROUTINE Nitrogen_DSMC_get_pressure (rhoi, temp, p)
      ...
      END SUBROUTINE Nitrogen_DSMC_get_pressure
  
      !------------------------------------------------------!
      ! This subroutine computes the mixture density.
      SUBROUTINE Nitrogen_DSMC_get_density (rhoi, rho)
      ...
      END SUBROUTINE Nitrogen_DSMC_get_density
  
      !------------------------------------------------------!
      ! This subroutine computes the specific gas constant R.
      SUBROUTINE Nitrogen_DSMC_get_Rgas (rhoi, R)
      ...
      END SUBROUTINE Nitrogen_DSMC_get_Rgas
  
      !------------------------------------------------------!
      ! This subroutine gives the species gas constants
      SUBROUTINE Nitrogen_DSMC_get_Ri (out_Ri)
      ...
      END SUBROUTINE Nitrogen_DSMC_get_Ri 
 
      !------------------------------------------------------!
      ! This subroutine gives the species molar masses
      SUBROUTINE Nitrogen_DSMC_get_mi (out_mi)
      ...
      END SUBROUTINE Nitrogen_DSMC_get_mi

      !------------------------------------------------------!
      ! This subroutine computes the mixture number density 
      SUBROUTINE Nitrogen_DSMC_get_mix_numb_dens (p, T, Te, xi, nb) 
      ...
      END SUBROUTINE Nitrogen_DSMC_get_mix_numb_dens 

      !------------------------------------------------------!
      ! This subroutine computes the species mass fractions
      SUBROUTINE Nitrogen_DSMC_get_mass_fractions (rho, rhoi, yi)
      ...
      END SUBROUTINE Nitrogen_DSMC_get_mass_fractions
  
      !------------------------------------------------------!
      ! This subroutine computes the species molar fractions
      SUBROUTINE Nitrogen_DSMC_get_molar_fractions (rhoi, xi)
      ...
      END SUBROUTINE Nitrogen_DSMC_get_molar_fractions 
      ...
      ! The mixture molar mass is also computed and given in output.
      SUBROUTINE Nitrogen_DSMC_mass_frac_from_molar_frac (xi, mm, yi)
      ...
      END SUBROUTINE Nitrogen_DSMC_mass_frac_from_molar_frac
      ...
      SUBROUTINE Nitrogen_DSMC_comp_tol(xi)
      ...
      END SUBROUTINE Nitrogen_DSMC_comp_tol

      !------------------------------------------------------!
      ! This subroutine computes the source term due to collisional and radiative processes.
      SUBROUTINE Nitrogen_DSMC_get_source (rhoi, temp, s) 
  
        USE mod_nitrogen_DSMC_CFD_source,         ONLY: source
      ...
      END SUBROUTINE Nitrogen_DSMC_get_source 
  
      !------------------------------------------------------!
      ! This subroutine computes the source term due to collisional and radiative processes 
      ! and its Jacobian (with respect to primitive variables).
      SUBROUTINE Nitrogen_DSMC_get_source_Jac (rhoi, temp, s, js) 
  
        USE mod_nitrogen_DSMC_CFD_source,         ONLY: source_Jac
      ...
      END SUBROUTINE Nitrogen_DSMC_get_source_Jac
 
      !------------------------------------------------------!
      ! This subroutine computes the species mass diffusion flux 
      SUBROUTINE Nitrogen_DSMC_compute_species_DiffFlux (p, T, Te, xi, diff_driv, Ji)

        USE mod_nitrogen_DSMC_CFD_transport,       ONLY: get_species_DiffFlux 
        ...
        CALL Nitrogen_DSMC_get_mix_numb_dens(p, T, Te, xi, nb) 
               
        ! Compute the species mass diffusion flux
        CALL get_species_DiffFlux (T, nb, xi, diff_driv, Ji)

      END SUBROUTINE Nitrogen_DSMC_compute_species_DiffFlux
 
      !------------------------------------------------------!
      ! This subrouotine computes the transport coefficients 
      SUBROUTINE Nitrogen_DSMC_compute_transpCoeff (p, xi, temp, mu, kappa, lambda, Di, chi)

        USE mod_nitrogen_DSMC_initialize_CFD,      ONLY: posTr, posTv
        USE mod_nitrogen_DSMC_CFD_transport,       ONLY: get_transport_coeff 
  
        REAL(KIND=8) :: nb
        REAL(KIND=8) :: lambda_tr, lambda_rot, lambda_vib
        REAL(KIND=8), DIMENSION(nb_ns) :: xi_tol
        ...
        xi_tol = xi
        CALL Nitrogen_DSMC_comp_tol(xi_tol)

        ! Compute mixture number density
        CALL Nitrogen_DSMC_get_mix_numb_dens(p, temp(1), temp(1), xi_tol, nb)  

        ! Compute the trasport coefficients
        CALL get_transport_coeff (nb, xi_tol, temp, mu, kappa, lambda_tr, lambda_rot, lambda_vib, Di, chi) 
      ...
      END SUBROUTINE Nitrogen_DSMC_compute_transpCoeff
#endif
  
  END MODULE mod_Nitrogen_DSMC_library
!------------------------------------------------------------------------------!  
