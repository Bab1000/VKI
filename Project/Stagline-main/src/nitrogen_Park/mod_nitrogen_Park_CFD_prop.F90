...
! when using the Park multi-temperature model.
  MODULE mod_nitrogen_Park_CFD_prop
  ...
  CONTAINS
    ...
    SUBROUTINE Q_int_rr_ho (T, Q)
      ...
      USE mod_nitrogen_Park_initialize_CFD,     ONLY: theta_vib, theta_rot
    ...
    SUBROUTINE compute_frozen_cv_T (T, cv)
      ...
      USE mod_nitrogen_Park_initialize_CFD,     ONLY: pos_N2, theta_vib, cv_tr, cv_rot, Rn2
    ...
    SUBROUTINE compute_frozen_cv_T_Tv (T, cv)
      ...
      USE mod_nitrogen_Park_initialize_CFD,     ONLY: pos_N2, theta_vib, cv_tr, cv_rot
    ...
    SUBROUTINE compute_frozen_cv_T_Tr_Tv (T, cv)
      ...
      USE mod_nitrogen_Park_initialize_CFD,     ONLY: theta_vib, cv_tr, cv_rot
    ...
    SUBROUTINE compute_frozen_energy_cv_T (T, cv, e)
      ...
      USE mod_nitrogen_Park_initialize_CFD,     ONLY: pos_N, pos_N2, theta_vib, cv_tr, cv_rot, Rn2, hf_N
    ...
    SUBROUTINE compute_frozen_energy_cv_T_Tv (T, cv, e)
      ...
      USE mod_nitrogen_Park_initialize_CFD,     ONLY: pos_N, pos_N2, theta_vib, cv_tr, cv_rot, hf_N
    ...
    SUBROUTINE ev_cvv_ho (Tv, ev, cvv)
      ...
      USE mod_nitrogen_Park_initialize_CFD,     ONLY: Rn2, theta_vib
    ...
    SUBROUTINE get_species_energy_cv (temp, e, cv)
      ...
      USE mod_nitrogen_Park_initialize_CFD,    ONLY: pos_N, pos_N2, nb_temp, nb_trot, theta_vib, hf_n, Rn2, & 
                                                  & cv_tr, cv_rot 
      ...
      ELSE
         ...
         IF (nb_trot.EQ.0) cv(pos_N2) = cv(pos_N2) + cv_rot       
    ...
    SUBROUTINE compute_T (rhoi, rho_eint, temp) 
      ...
      USE mod_nitrogen_Park_initialize_CFD,    ONLY: nb_ns, pos_N, pos_N2, hf_n, Rn, Rn2, theta_vib, & 
                                                   & cv_tr  
      ...
      ! Newton's loop starts here
      DO WHILE ((res.GT.tol).AND.(it.LE.it_max))
      ...
      IF (it.GT.it_max) THEN 
         PRINT*
         WRITE(*,10)'solver_fvmcc_f90:excessive number of Newton iterators, in mod_nitrogen_Park_CFD_prop...'
         PRINT*
         WRITE(*,20)'T ',T,'Told',Told,'res ',res
         PRINT*
         PAUSE
      ENDIF

10  FORMAT(A)
20  FORMAT(A,E14.6,1X,A,E14.6,1X,A,E14.6)
    ...
    SUBROUTINE compute_T_Tv (rhoi, rho_eint, temp) 
      ...
      USE mod_nitrogen_Park_initialize_CFD,    ONLY: pos_N, pos_N2, nb_ns, cv_rot, theta_vib, Rn2, Rn, hf_n, cv_tr 
      ...
      b = 0.d0
      DO is = 1,nb_ns 
        b = b + rhoi(is)*cv_tr(is)
      ENDDO
      b = b + rhoi(pos_N2)*cv_rot
    ...
    SUBROUTINE compute_T_Tr_Tv (rhoi, rho_eint, temp) 
      ...
      USE mod_nitrogen_Park_initialize_CFD,    ONLY: pos_N, pos_N2, nb_ns, cv_tr, theta_vib, Rn2, Rn, hf_n 
    ...
    SUBROUTINE post_shock_neq (p1, u1, T1, p2, u2, T2, yi, xN)
      ...
      USE mod_nitrogen_Park_initialize_CFD,         ONLY: nb_ns, nb_temp, nb_trot, pos_N, pos_N2, urg, & 
                                                        & cv_rot, cv_tr, mi, Ri, solver
      ...
      mass = 0.d0
      DO is = 1,nb_ns 
         mass = mass + xi(is)*mi(is)
      ENDDO
      R = urg/mass
     
      ! Species mass fractions
      DO is = 1,nb_ns 
         yi(is) = xi(is)*mi(is)/mass
      ENDDO
      ...
      IF (nb_temp.EQ.1) THEN
         ...
         WRITE(*,5)solver(1:length),':: Nitrogen Park -> thermal equilibrium post-shock conditions'
         PRINT*
      ...
      ELSE 
         ...
         IF (nb_trot.EQ.0) THEN 
            ...
            DO is = 1,nb_ns 
               ...
               tmp2 = cv_tr(is)
               cv   = cv + tmp1*tmp2
               cp   = cp + tmp1*(tmp2 + Ri(is))
            ENDDO
            tmp1 = yi(pos_N2)*cv_rot
            cv   = cv + tmp1
            cp   = cp + tmp1
...
15  FORMAT(A,E14.6,A)
  ...
  END MODULE mod_nitrogen_Park_CFD_prop
!------------------------------------------------------------------------------!
