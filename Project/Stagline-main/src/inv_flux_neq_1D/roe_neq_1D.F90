...
!! proposed by Roe for a 1D nonequilibrium gas flows.    
  SUBROUTINE roe_neq_1D (nx, left_data, right_data, u_left, u_right, f)

#include "../config.h"
 
    USE mod_general_data,             ONLY: nb_ns, nb_eq, nb_temp, nb_int_temp, nb_te, pos_u_cell, pos_pres_cell,  & 
                                          & pos_c_cell, pos_rhou, pos_rhoE, pos_rhoek
    USE mod_numerics_data,            ONLY: deltau, lambda, left_eig, right_eig, absA_eig, diss, cons_Roe, phys_data_Roe
    USE mod_function_pointer,         ONLY: eigsys_neq_1D, roe_avg_neq_1D
    ...
    ! Global momentum and energy equations (pressure contribution is added)
    ...
    ! Eigensystem 
    CALL eigsys_neq_1D (nx, cons_Roe, phys_data_Roe, lambda, right_eig, left_eig)
    ...
    IF (nb_temp.GT.1) THEN
       ...
       IF (tmp3.LE.eps) THEN
         DO k = 1,nb_int_temp + nb_te
            lambda(pos_rhoek + k - 1) = eps
         ENDDO
       ELSE 
         DO k = 1,nb_int_temp + nb_te
            lambda(pos_rhoek + k - 1) = tmp3
         ENDDO
       ENDIF
