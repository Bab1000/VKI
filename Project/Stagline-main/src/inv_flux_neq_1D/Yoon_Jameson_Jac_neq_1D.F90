!------------------------------------------------------------------------------!
! This subrotine provides the numerical flux Jacobian for 1D nonequilibrium flows
! according to the approximation proposed by Yoon and Jameson.
  SUBROUTINE Yoon_Jameson_Jac_neq_1D (nx, left_data, right_data, u_left, u_right, jfl, jfr)

    USE mod_general_data,             ONLY: nb_eq, pos_u_cell, pos_c_cell
    USE mod_function_pointer,         ONLY: inv_flux_Jac_neq_1D
    ...
    REAL(KIND=8), DIMENSION(:,:), INTENT(OUT) :: jfl, jfr   
    ...
    IF (Mnl.GE.1.d0) THEN
       ...
       ! Supersonic flow
       CALL inv_flux_Jac_neq_1D(nx, u_left, left_data, jfl) 
    ...
    IF (Mnr.LE.-1.d0) THEN
       ...
       ! Supersonic flow
       CALL inv_flux_Jac_neq_1D(nx, u_right, right_data, jfr) 
  ...
  END SUBROUTINE Yoon_Jameson_Jac_neq_1D
!------------------------------------------------------------------------------!
