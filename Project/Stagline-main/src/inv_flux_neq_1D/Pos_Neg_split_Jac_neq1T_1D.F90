...
!! In this case the flow is characterized by one single temperature T.
  SUBROUTINE Pos_Neg_split_Jac_neq1T_1D (nx, left_data, right_data, u_left, u_right, jfl, jfr)

    USE mod_general_data,             ONLY: nb_ns, pos_alpha_cell, pos_c_cell, pos_ei_cell, pos_ek_cell,        &
                                          & pos_gamma_cell, pos_h0_cell, pos_rho_cell, pos_T_cell, pos_u_cell,  & 
                                          & pos_rhou, pos_rhoE, yi, epsi, sigmai, Ri
    ...
    REAL(KIND=8) :: c, cn, ek, h0, rho, u, u_vn, vn, vn2, T, p
    ...
    REAL(KIND=8) :: eig_diff, eig_sum, eig_diff_vn_ov_c, eig_sum_m_l1p, eig_sum_m_l1m, u_eig_sum_m_l1p,         &
                  & h0_eig_sum_m_l1p, u_eig_sum_m_l1m, h0_eig_sum_m_l1m, eig_sum_vn_nx, eig_diff_ov_c,          & 
                  & eig_diff_nx_ov_c, eig_sum_vn2, u_eig_sum_m_l1p_alpha_ov_c2, u_eig_sum_m_l1m_alpha_ov_c2 
  ...
  END SUBROUTINE Pos_Neg_split_Jac_neq1T_1D
!------------------------------------------------------------------------------!
