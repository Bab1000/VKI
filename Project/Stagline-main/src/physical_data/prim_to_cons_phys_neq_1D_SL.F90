...
!! 1D stagnation line nonequilibrium flows gas flows when solving the Euler equations
  SUBROUTINE prim_to_cons_phys_neq_1D_SL_Eu (prim, cons, phys_prop)

    USE mod_general_data,                 ONLY: nb_ns, nb_temp, nb_int_temp, pos_u, pos_v, pos_T, pos_rhou, &
                                              & pos_rhov, pos_rhoE, pos_u_cell, pos_v_cell, pos_T_cell,     & 
                                              & pos_alpha_cell, pos_beta_cell, pos_pres_cell, pos_c_cell,   & 
                                              & pos_gamma_cell, pos_rho_cell, pos_h0_cell, pos_ei_cell,     & 
                                              & pos_ek_cell, rhoi, ei, beta, temp, rho_eint, nb_te
  ...
  END SUBROUTINE prim_to_cons_phys_neq_1D_SL_Eu 
!------------------------------------------------------------------------------!
!> This subroutine computes the conservative and physical variables from the primitive variables for 
!! 1D stagnation line nonequilibrium flows gas flows when solving the Navier-Stokes equations
  SUBROUTINE prim_to_cons_phys_neq_1D_SL_Ns (prim, cons, phys_prop)

    USE mod_general_data,                 ONLY: nb_ns, nb_temp, nb_int_temp, pos_u, pos_v, pos_T, pos_rhou, & 
                                              & pos_rhov, pos_rhoE, pos_u_cell, pos_v_cell, pos_T_cell,     & 
                                              & pos_alpha_cell, pos_beta_cell, pos_pres_cell, pos_c_cell,   & 
                                              & pos_gamma_cell, pos_rho_cell, pos_h0_cell, pos_ei_cell,     & 
                                              & pos_ek_cell, pos_mu_cell, pos_Di_cell, pos_lambda_cell,     & 
                                              & pos_kappa_cell, pos_chi_cell, rhoi, ei, beta, temp,         & 
                                              & rho_eint, Di, lambda_vec, chi, xi, nb_te
    USE mod_neq_function_pointer,         ONLY: library_get_thermodynamic_data, library_get_energy_densities, &
  ...
  END SUBROUTINE prim_to_cons_phys_neq_1D_SL_Ns 
!------------------------------------------------------------------------------!
