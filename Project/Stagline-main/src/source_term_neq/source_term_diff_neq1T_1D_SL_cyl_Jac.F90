...
!! for nonequilibrium flows (cylinder case - 1 temperature).
  SUBROUTINE source_term_diff_neq1T_1D_SL_cyl_Jac(r_c, vol_l, vol_c, vol_r, prop_left, prop_cell, prop_right, & 
    ...
    REAL(KIND=8) :: beta, ek, lambda, mu, mm, p, rho, rhoi_Vdi, Vdiff, u, v, uv, us, vs, vel_sum, T
    ...
    REAL(KIND=8) :: ov_rho, mu_ov_rho, m2_mu_ov_rho, m3_mu_ov_rho, m4_mu_ov_rho, m6_mu_ov_rho, lam_ov_beta
    ...
    tmp1 = - mu_ov_rho*(vel_sum + u)
    tmp2 = - coeff1_mu_ov_rho*(2.d0*u + 3.d0*v)
    tmp3 = - coeff0*mu_ov_rho*u*(vel_sum + 3.d0*u)
    ...
    Ad(pos_rhoE,pos_rhou) = coeff1_mu_ov_rho*(2.d0*u - v) - lam_ov_beta*u
    ...
    tmp1 = m3_mu_ov_rho*vel_sum
    tmp2 = coeff2_mu_ov_rho*vel_sum
    tmp3 = coeff1_mu_ov_rho*(5.d0*us + uv - 4.d0*vs)
  ...
  END SUBROUTINE source_term_diff_neq1T_1D_SL_cyl_Jac
!------------------------------------------------------------------------------!
