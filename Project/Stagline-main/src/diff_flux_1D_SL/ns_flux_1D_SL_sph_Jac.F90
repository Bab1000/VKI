...
!! for a calorically perfect gas (sphere case).
  SUBROUTINE ns_flux_1D_SL_sph_Jac (r_l, r_r, vol_l, vol_r, left_data, right_data, u_left, u_right, fd, jfdl, jfdr)

    USE mod_general_data,               ONLY: pos_u_cell, pos_v_cell, pos_T_cell, pos_mu_cell, pos_lambda_cell, & 
                                            & pos_rho_cell, pos_rho, pos_rhou, pos_rhov, pos_rhoE, R_gas, gamma
    ...
    ! Velocity components, temperature, density, dynamic viscosity and thermal conductivity of left and right states
    ! Left state
    ...
    Ad(pos_rhou,pos_rho) = - coeff*u_mu_ov_rho 
    Ad(pos_rhov,pos_rho) = - v_mu_ov_rho
    Ad(pos_rhoE,pos_rho) = - coeff*us_mu_ov_rho + 0.5d0*lam_ov_rhoR_gm1*us - lam_ov_rho_T
 
    ! Second column 
    Ad(pos_rho,pos_rhou)  = 0.d0
    Ad(pos_rhou,pos_rhou) = coeff*mu_ov_rho
    Ad(pos_rhov,pos_rhou) = 0.d0
    Ad(pos_rhoE,pos_rhou) = coeff*u_mu_ov_rho - lam_ov_rhoR_gm1*u
    ...
    vel_sum   = (u + v)
    mu_ov_rho = mu/(rho*r) 
    coeff_mu_ov_rho = coeff*mu_ov_rho
  ...
  END SUBROUTINE ns_flux_1D_SL_sph_Jac
!------------------------------------------------------------------------------!
!------------------------------------------------------------------------------!
!> This subroutine computes the diffusive flux and the related Jacobians when solving the 1D stagnation line equations 
!! for a calorically perfect gas and accounting for the mesh metrics (sphere case). (A. Turchi)
  SUBROUTINE ns_flux_1D_SL_sph_Jac_metr (r_l, r_r, vol_l, vol_r, left_data, right_data, u_left, u_right, fd, jfdl, jfdr)
    ...
    rho = (rhol *vol_l+ rhor*vol_r)/(vol_l+vol_r)
  ...
  END SUBROUTINE ns_flux_1D_SL_sph_Jac_metr
!------------------------------------------------------------------------------!
