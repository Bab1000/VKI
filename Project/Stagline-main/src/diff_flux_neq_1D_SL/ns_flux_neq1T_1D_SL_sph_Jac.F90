...
!! for nonequilibrium flows (sphere case - 1 temperature).
  SUBROUTINE ns_flux_neq1T_1D_SL_sph_Jac (r_l, r_r, vol_l, vol_r, left_data, right_data, u_left, u_right, fd, jfdl, jfdr)
    ...
    USE mod_general_data,            ONLY: nb_ns, nb_dim, pos_u_cell, pos_v_cell, pos_ei_cell, pos_T_cell,  & 
                                         ...
                                         & xi, yi, Ri, ei, hi, mi, Dij_mat, Di, diff_driv, Ji, nb_temp, chi
  ...
  END SUBROUTINE ns_flux_neq1T_1D_SL_sph_Jac
!------------------------------------------------------------------------------!
!------------------------------------------------------------------------------!
!------------------------------------------------------------------------------!
!> This subroutine computes the diffusive flux and the related Jacobians when solving the 1D stagnation line equations 
!! for nonequilibrium flows accounting for metrics (sphere case - 1 temperature) (A. Turchi).
  SUBROUTINE ns_flux_neq1T_1D_SL_sph_Jac_metr (r_l, r_r, vol_l, vol_r, left_data, right_data, u_left, u_right, fd, jfdl, jfdr)
    ...
    beta = (betal *vol_l+ betal*vol_r)/(vol_l+vol_r)
    lambda = (lambdal *vol_l+ lambdar*vol_r)/(vol_l+vol_r) 

    ! Species densities, average binary diffusion coefficients and specific energies at cell interface  
    rho = 0.d0
    DO i = 1,nb_ns 
       dens = (rhoil(i)*vol_l + rhoir(i)* vol_r) /(vol_l+vol_r)
       rho  = rho + dens
       rhoi(i) = dens 
       tmp1   = (left_data(pos_ei_cell + i - 1)*vol_l + right_data(pos_ei_cell + i - 1)*vol_r) /(vol_l+vol_r)
       ei(i)   = tmp1
       hi(i)   = tmp1 + Ri(i)*T
    ENDDO
  ...
  END SUBROUTINE ns_flux_neq1T_1D_SL_sph_Jac_metr
!------------------------------------------------------------------------------!
