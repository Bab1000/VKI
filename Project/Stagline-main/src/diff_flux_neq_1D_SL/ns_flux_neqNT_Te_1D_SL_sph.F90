...
!! for nonequilibrium flows (sphere case - N temperatures and one temperature for free electrons Te).
  SUBROUTINE ns_flux_neqNT_Te_1D_SL_sph (r_l, r_r, vol_l, vol_r, left_data, right_data, u_left, u_right, fd)
    ...
    USE mod_general_data,            ONLY: nb_ns, nb_dim, nb_temp, nb_int_temp, pos_u_cell, pos_v_cell,  & 
                                         ...
                                         & rhoi, xi, Ri, ei, diff_driv, Ji, lambda_vec, nb_te, pos_em
    ...
    REAL(KIND=8) :: tmp
    REAL(KIND=8) :: ov_dr
    REAL(KIND=8) :: r, u, v, lambda, mu, p, q_intk, rhoi_Vdi, Vdiff, T, Te
    REAL(KIND=8) :: tau_rr, tau_rt, tau_tt
    REAL(KIND=8) :: mul, mur, pl, pr, Tl, Tr, ul, ur, vl, vr, Tel, Ter
    ...
    Tel = left_data(pos_T_cell+nb_temp-1)
    ...
    Ter = right_data(pos_T_cell+nb_temp-1)
    ...
    ! Specific enthalpies at cell interface 
    ...
    DO i = 1,nb_ns
       IF (i==pos_em) THEN
         ei(i) = ei(i) + Ri(i)*Te
         ei((nb_temp-1)*nb_ns+i) = ei((nb_temp-1)*nb_ns+i) + Ri(i)*Te
       ELSE
         ei(i) = ei(i) + Ri(i)*T
       ENDIF
    ENDDO
  ...
  END SUBROUTINE ns_flux_neqNT_Te_1D_SL_sph
!------------------------------------------------------------------------------!
!------------------------------------------------------------------------------!
!> This subroutine computes the diffusive flux when solving the 1D stagnation line equations 
!! for nonequilibrium flows accounting for the mesh metrics (sphere case - N temperatures and one temperature for free electrons Te).
  SUBROUTINE ns_flux_neqNT_Te_1D_SL_sph_metr (r_l, r_r, vol_l, vol_r, left_data, right_data, u_left, u_right, fd)
    ...
    Te  = (Tel*vol_l + Ter*vol_r)/(vol_l+vol_r)
  ...
  END SUBROUTINE ns_flux_neqNT_Te_1D_SL_sph_metr
!------------------------------------------------------------------------------!
