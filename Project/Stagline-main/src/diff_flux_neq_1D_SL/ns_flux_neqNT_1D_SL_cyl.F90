...
!! for nonequilibrium flows (cylinder case - N temperatures).
  SUBROUTINE ns_flux_neqNT_1D_SL_cyl (r_l, r_r, vol_l, vol_r, left_data, right_data, u_left, u_right, fd)
    ...
    REAL(KIND=8) :: tmp
    REAL(KIND=8) :: ov_dr
    REAL(KIND=8) :: r, u, v, lambda, mu, p, q_intk, rhoi_Vdi, T
    REAL(KIND=8) :: tau_rr, tau_rt, tau_tt
    REAL(KIND=8) :: mul, mur, pl, pr, Tl, Tr, ul, ur, vl, vr
    REAL(KIND=8) :: du_dr, dv_dr
    REAL(KIND=8) :: q, q_Diff, q_Diff_int, q_Fourier_tr, q_Fourier_int, q_Therm_Diff
  ...
  END SUBROUTINE ns_flux_neqNT_1D_SL_cyl
!------------------------------------------------------------------------------!
