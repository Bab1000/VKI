...
def plot_posteriors_individual(idata_dict, pressure_labels,
    ...
    # Panerai model (approximate values in log10)
    panerai_model = {
        1500: np.log10(0.100),   # 15 mbar
        5000: np.log10(0.010),   # 50 mbar
        10000: np.log10(0.010),  # 100 mbar
        20000: np.log10(0.005)   # 200 mbar 
    }

    # Viladegut & Chazot model (exact pressure match)
    viladegut_model = {
        1500: np.log10(0.08820),  # 15 mbar
        5000: np.log10(0.02661),  # 50 mbar
        10000: np.log10(0.00960), # 100 mbar
        20000: np.log10(0.00636)  # 200 mbar
}
    ...
    for p in pressure_labels:
        ...
        # Ajouter les valeurs Panerai et Viladegut
        if p in panerai_model:
            ax.axvline(panerai_model[p], color="black", linestyle="--", linewidth=1.5, label=r"Panerai $(\gamma_{ref})$")

        if p in viladegut_model:
            ax.axvline(viladegut_model[p], color="orange", linestyle="-.", linewidth=1.5, label=r"Viladegut & Chazot $(\gamma_{ref})$")
        ...
        ax.legend(by_label.values(), by_label.keys(), fontsize=14, loc='upper left')
...
def MAP_comparison_plot_individual(sm_model,
                                   ...
                                   filename_base="Models_predictions_comp"):
    """
    Generates one figure per pressure bin (1500, 5000, 10000 Pa), showing:
    - Experimental data (black circles)
    - MAP prediction from SSG model (blue squares)
    - MAP prediction from UG model (green triangles)
    - Prediction using Panerai gamma_ref model (red diamonds)
    - Prediction using Viladegut & Chazot gamma_ref model (purple X markers)

    Each figure is saved separately with suffix _<pressure>mbar.
    """

    # gamma_ref values (log10) from literature models
    panerai_model = {
        1500: np.log10(0.100),   # 15 mbar
        5000: np.log10(0.010),   # 50 mbar
        10000: np.log10(0.010),  # 100 mbar
        20000: np.log10(0.005)   # 200 mbar (optional)
    }

    viladegut_model = {
        1500: np.log10(0.08820),
        5000: np.log10(0.02661),
        10000: np.log10(0.00960),
        20000: np.log10(0.00636)
    }

    print(Fore.BLUE + "[STEP] MAP Comparison Plot — one figure per pressure bin")

    pressure_bins = [1500, 5000, 10000]
    colors = {
        'exp': 'black', 'ssg': '#1f77b4', 'ug': '#2ca02c',
        'panerai': 'red', 'viladegut': 'purple'
    }
    markers = {
        'exp': 'o', 'ssg': 's', 'ug': '^',
        'panerai': 'D', 'viladegut': 'X'
    }
    alpha = 0.9
    cache = {}

    for bin_p in pressure_bins:
        fig, ax = plt.subplots()
        plotted_labels = {
            'exp': False, 'ssg': False, 'ug': False,
            'panerai': False, 'viladegut': False
        }

        for i in range(len(T_test)):
            try:
                pstat_i = Pstat_test[i]
                bin_i = min(pressure_bins, key=lambda x: abs(x - pstat_i))
                if bin_i != bin_p:
                    continue
                ...
                ax.errorbar(
                    ...
                    markersize=6,
                    alpha=alpha,
                    label="Experimental values" if not plotted_labels["exp"] else ""
                )
                plotted_labels["exp"] = True
                ...
                ax.errorbar(
                    ...
                    markersize=8,
                    alpha=alpha,
                    label="Predictions SSG" if not plotted_labels["ssg"] else ""
                )
                plotted_labels["ssg"] = True
                ...
                ax.errorbar(
                    ...
                    markersize=8,
                    alpha=alpha,
                    label="Predictions UG" if not plotted_labels["ug"] else ""
                )
                plotted_labels["ug"] = True

                # Panerai prediction
                if bin_p in panerai_model:
                    gamma_pan = panerai_model[bin_p]
                    pred_pan = sm_model.predict_values(np.array([
                        Pdyn_test[i], Pstat_test[i], T_test[i],
                        gamma_pan, gamma_pan
                    ]).reshape(1, -1))[0, 0] / 1000
                    ...
                    ax.scatter(
                        T_test[i], pred_pan,
                        color=colors["panerai"],
                        marker=markers["panerai"],
                        s=80,
                        label=r"Panerai $(\gamma_{ref})$" if not plotted_labels["panerai"] else ""
                    )
                    plotted_labels["panerai"] = True

                # Viladegut prediction
                if bin_p in viladegut_model:
                    gamma_vil = viladegut_model[bin_p]
                    pred_vil = sm_model.predict_values(np.array([
                        Pdyn_test[i], Pstat_test[i], T_test[i],
                        gamma_vil, gamma_vil
                    ]).reshape(1, -1))[0, 0] / 1000
                    ...
                    ax.scatter(
                        T_test[i], pred_vil,
                        color=colors["viladegut"],
                        marker=markers["viladegut"],
                        s=80,
                        label=r"Viladegut & Chazot $(\gamma_{ref})$" if not plotted_labels["viladegut"] else ""
                    )
                    plotted_labels["viladegut"] = True
        ...
        ax.set_title(f"{bin_p / 100:.0f} mbar", fontsize=16)
        ax.grid(True, linestyle='--', linewidth=0.5, alpha=0.7)

        # Legend (automatic, deduplicated)
        handles, labels = ax.get_legend_handles_labels()
        by_label = dict(zip(labels, handles))
        ax.legend(by_label.values(), by_label.keys(), fontsize=12, loc='upper left')

        # Save figure
        fig_name = f"{filename_base}_{bin_p/100}mbar.png"
...
def evaluate_pan_and_vil_models(sm_model, Pdyn_list, Pstat_list, T_list, HF_exp_list):
    """
    Évalue les erreurs des modèles gamma constants de Panerai et Viladegut.
    Retourne les prédictions et les erreurs pour chaque modèle.
    """
    ...
    gamma_models = {
        "Panerai": {
            1500: np.log10(0.100),
            5000: np.log10(0.010),
            10000: np.log10(0.010),
            20000: np.log10(0.005)
        },
        "Viladegut": {
            1500: np.log10(0.08820),
            5000: np.log10(0.02661),
    ...
    for name, model in gamma_models.items():
        print(Fore.BLUE + f"\n[STEP] Evaluation using {name} model")
        y_pred = []
        ...
        for i in range(len(HF_exp_list)):
            ...
            bin_pstat = int(round(pstat_i / 100) * 100)
            closest_bin = min(model.keys(), key=lambda x: abs(x - bin_pstat))
            gamma_log10 = model[closest_bin]

            input_vector = np.array([
                pdyn_i, pstat_i, temp_i, gamma_log10, gamma_log10
            ]).reshape(1, -1)

            pred = sm_model.predict_values(input_vector)[0, 0] / 1000
            y_pred.append(pred)

            print(Fore.CYAN + f"[{i}] Pstat={pstat_i:.0f} → bin={closest_bin}, log10(gamma)={gamma_log10:.4f}, HF_pred={pred:.3f}, HF_exp={HF_exp_list[i]:.3f}")
        ...
        print(Fore.YELLOW + f"\n {name} Error Summary:")
        print(f"{name} → MAPE = {mape_val:.2f} %, MAE = {mae_val:.3f} kW/m²")
