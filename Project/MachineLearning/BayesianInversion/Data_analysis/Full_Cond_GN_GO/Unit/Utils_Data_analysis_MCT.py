...
def MAP_plots(sm_model, samples, targ_p, Pstat_test, Pdyn_test, T_test,
...
def MAP_plots_Unit_G(sm_model, samples, targ_p, MF_test, Pdyn_test, T_test,
              T_exp_list, HF_exp_list, HF_exp_err_list,
              output_path, global_output_path):
    """
    Loops over all posterior samples and plots MAP predictions vs experimental data.
...
def G_plotting(samples,targ_p,global_output_path):
    try:
        G = samples["Gglob"].values.flatten()
    except Exception as e:
        print(f"[ERROR] Cannot extract GN/GO from samples: {e}")

    if G.size > 0:
        # Create 1D KDEs
        kde_g = gaussian_kde(G)
        ...
        # Support of the prior
        x_prior = [min_gamma, min_gamma + delta_gamma]  # [-4, 0]

        # Constant density value for uniform distribution
        y_prior = [1 / delta_gamma, 1 / delta_gamma]  # [0.25, 0.25]

        # Plot both KDEs
        # Plot
        plt.figure(figsize=(8, 5))
        plt.plot(x_grid, y_g, color="#4A90E2", label="GN")  # bleu pastel
        plt.fill_between(x_grid, y_g, color="#4A90E2", alpha=0.3)
        ...
        plt.grid(True)
        plt.tight_layout()
        plt.savefig(global_output_path + f"/Gamma_plot/{targ_p/100}.jpeg", format='jpeg', dpi=300, bbox_inches='tight')
    else:
        print("[WARNING] One or both of GN and GO arrays are empty.")
...
def plot_relative_errors(sm_model, samples, targ_p, T_test, HF_exp_list, output_path):
    """
    Plots the relative error (%) between predicted and experimental heat flux
    for each test case, using line plots colored by Pstat.
    Also displays weighted average relative error in %.
    """
    import matplotlib.pyplot as plt
    import numpy as np
    import os

    print(Fore.BLUE + "[STEP] Plotting relative errors (MAP vs Experimental)")

    # === Setup pression statique (catégorique) ===
    pstat_color_dict = {1500: 'blue', 5000: 'green', 10000: 'red'}
    pstat_marker_dict = {1500: 's', 5000: 'D', 10000: '^'}

    # === Group data by pressure ===
    grouped_data = {1500: [], 5000: [], 10000: []}
    ...
    for i in range(len(T_test)):
        try:
            ...
            # Identifier Pstat moyenne et classer
            avg_pstat = int(round(Pstat_samples.mean() / 100) * 100)
            closest_pstat = min(grouped_data.keys(), key=lambda x: abs(x - avg_pstat))
            grouped_data[closest_pstat].append((T_test[i], rel_error))

        except Exception as e:
            print(Fore.RED + f"[ERROR] Failed at point {i}: {e}")
            continue
    ...
    for pstat, values in grouped_data.items():
        if values:
            values.sort()
            ...
            plt.plot(
                temps,
                errors,
                label=f"Pstat = {pstat} Pa",
                color=pstat_color_dict[pstat],
                marker=pstat_marker_dict[pstat],
                linewidth=2,
                markersize=6
            )
