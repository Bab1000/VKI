...
def plot_relative_errors_by_massflow(sm_model, samples, targ_p, T_test, HF_exp_list, MF_test, output_path):
    ...
    for i in range(len(T_test)):
        try:
            ...
            Pstat_samples = samples[f"Pstat_TC={i}"].values.flatten()
            T_samples = samples[f"T_TC={i}"].values.flatten()
            Gglob_samples = samples["Gglob"].values.flatten()

            # === Calcul MAP
            min_len = min(len(Pdyn_samples), len(Pstat_samples), len(T_samples), len(Gglob_samples))
            predictions = []
            for idx in range(min_len):
                XV = np.array([
                    Pdyn_samples[idx],
                    ...
                    T_samples[idx],
                    Gglob_samples[idx],
                    Gglob_samples[idx]  # utilisé deux fois comme prévu
                ]).reshape(1, -1)
                pred = sm_model.predict_values(XV)[0, 0] / 1000
                predictions.append(pred)
