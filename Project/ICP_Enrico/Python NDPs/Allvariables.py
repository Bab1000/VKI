...
# -*- coding: utf-8 -*-
"""
Created on Fri Apr 14 16:39:01 2023
...
os.chdir('/nobackup/rm/vromano/ICP simulations/HS50 probe/')           # Set the folder where there are the simulations

# Create csv file
f = open('Results/AllVariables_' + str(math.trunc(ps/100)) + 'mbar.csv', 'w')
writer = csv.writer(f)
writer.writerow(['mdot [g/s]', 'ps [mbar]', 'Pel [kW]', 'u_Torchexit [m/s]', 'beta_Torchexit [1/s]', 'T_Torchexit [K]', 'p_Torchexit [Pa]', 'rho_Torchexit [kg/m^3]', 'h_Torchexit [J/kg]', 'u_freestream [m/s]', 'beta_freestream [1/s]', 'T_freestream [K]', 'p_freestream [Pa]', 'rho_freestream [kg/m^3]', 'h_freestream [J/kg]', 'delta [m]','u_delta [m/s]', 'beta_delta [1/s]', 'dbetadx_delta [1/ms]', 'T_delta [K]', 'p_delta [Pa]', 'rho_delta [kg/m^3]', 'h_delta [J/kg]', 'u_wall [m/s]', 'beta_wall [1/s]', 'T_wall [K]', 'p_wall [Pa]', 'rho_wall [kg/m^3]', 'h_wall [J/kg]', 'NDP1', 'NDP2', 'NDP3', 'NDP4', 'NDP5'])
f.close()

#f = open('Results/AllVariables.csv', 'w')
#writer = csv.writer(f)
#writer.writerow(['mdot [g/s]', 'ps [mbar]', 'Pel [kW]', 'u_Torchexit [m/s]', 'beta_Torchexit [1/s]', 'T_Torchexit [K]', 'p_Torchexit [Pa]', 'rho_Torchexit [kg/m^3]', 'h_Torchexit [J/kg]', 'u_freestream [m/s]', 'beta_freestream [1/s]', 'T_freestream [K]', 'p_freestream [Pa]', 'rho_freestream [kg/m^3]', 'h_freestream [J/kg]', 'delta [m]','u_delta [m/s]', 'beta_delta [1/s]', 'dbetadx_delta [1/ms]', 'T_delta [K]', 'p_delta [Pa]', 'rho_delta [kg/m^3]', 'h_delta [J/kg]', 'u_wall [m/s]', 'beta_wall [1/s]', 'T_wall [K]', 'p_wall [Pa]', 'rho_wall [kg/m^3]', 'h_wall [J/kg]', 'NDP1', 'NDP2', 'NDP3', 'NDP4', 'NDP5'])
#f.close()
...
################################ Write in a file ###########################################   
    ...
    f = open('Results/AllVariables_' + str(math.trunc(ps/100)) + 'mbar.csv', 'a')
    writer = csv.writer(f)
    writer.writerow(result)
    f.close()

#    f = open('Results/AllVariables.csv', 'a')
#    writer = csv.writer(f)
#    writer.writerow(result)
#    f.close()
...
############################## Plot ########################################################
    ...
    plt.plot([x_BLfitted, x_BLfitted], [np.min(beta/10**4), np.max(beta/10**4)], '--k')
    # plt.xlabel('x [m]')
    ...
    plt.xlim(0.82,0.890)
    plt.ylim(-1, dbetadx[BL_index]/10**6+0.5)
    # plt.grid()
    plt.tight_layout()
