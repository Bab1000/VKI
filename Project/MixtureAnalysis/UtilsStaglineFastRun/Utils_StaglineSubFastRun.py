...
def InputFileGenerator(stagline_simulations_path, input_template_path, sim_name, inputs, init_cond,mixture,air_5_restart):
    ...
    if air_5_restart == ".TRUE.":
        RestartFromPreviousSim(stagline_simulations_path,sim_name,mixture)
...
def RestartFromPreviousSim(stagline_simulations_global_path, sim_name, mixture):
    ...
    file1_list = glob.glob(os.path.join(old_sim_folder, pattern1))
    file2_list = glob.glob(os.path.join(old_sim_folder, pattern2))
...
def RunStagline(sim_folder_path,sim_name,CFL_range,Iter,cfl_inter):
