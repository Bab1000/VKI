...
def UserInputsProcessing(Txin, Tyin, pc, mixture, pdyn, uin, vin, R, n_species, cfl_val, Twall, cfl_inter, cfl_adaptive, Log_CFL,residual,restart):
...
def InputFileGenerator(stagline_simulations_path, input_template_path, sim_name, inputs, init_cond,mixture):
    """
...
def RunStagline(sim_folder_path,sim_name):
    ...
    while process.poll() is None:  # While the process is running
        if os.path.exists(data_file):
            try:
                ...
                print(Fore.YELLOW + f"[WARNING] Could not read file: {e}")
