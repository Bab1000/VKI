!------------------------------------------------------------------------------!
!> This subroutine writes the solution for 1D nonequilibrium flow simulations.
!! An additional file (restart.dat) is written in order to enable a re-start from a previous solution.
  SUBROUTINE write_sol_neq_1D ()

    USE mod_general_data,    ONLY: nb_eq, nb_ns, nb_temp, nb_prop, nb_cells, pos_u_cell, pos_pres_cell,  & 
                                 & pos_T_cell, pos_rho_cell, pos_c_cell, pos_h0_cell, start_u_phys,      & 
                                 & start_prop_phys, xc, u, cell_prop, flag_extra_data, simulation_id,    &
                                 & ios_dir
    ...
    IF (flag_extra_data.EQV..TRUE.) THEN
        ...
        WRITE(out1,10)'# xc  rhoi  u  T  Tint  Te  p  rho  h0  Mf'
    ELSE 
        WRITE(out1,10)'# Nonequilibrium flow'
        WRITE(out1,10)'# Primitive variables'
        WRITE(out1,10)'# xc  rhoi  u  T  Tint  Te'
    ENDIF
    ...
    IF (flag_extra_data.EQV..TRUE.) THEN
       ...
       DO i = 1,nb_cells
          ...
          WRITE(out1,20)xc(i + 2),cons(1:nb_ns),cell_data(pos_u_cell),cell_data(1:nb_temp), & 
                      & cell_data(pos_pres_cell),cell_data(pos_rho_cell),cell_data(pos_h0_cell),Mach
          WRITE(out2,20)cons(1:nb_ns),cell_data(pos_u_cell),cell_data(1:nb_temp)
...
20  FORMAT(10000(E20.10,1X))
  ...
  END SUBROUTINE write_sol_neq_1D
!------------------------------------------------------------------------------!
