...
! when using the NASA Ames ab-initio database.
  MODULE mod_nitrogen_NASA_CFD_source_BRVC

#define pre_KACHE

    IMPLICIT NONE

    REAL(KIND=8), PARAMETER :: xi_tol = 1.d-50
    ...
    CONTAINS
      ...
      ! This subroutine is called when using the BRVC model. This subroutine ca be used also for computing
      ...
      SUBROUTINE source_BRVC (rhoi, temp, omega)

        USE mod_nitrogen_NASA_initialize_CFD,        ONLY: nb_ns, nb_bins, nb_eq, mm_N, mm_N2, ad_kf, bd_kf, cd_kf,   &  
                                                        &  ae_kf, be_kf, ce_kf, ap_kf, bp_kf, cp_kf, T_store, T_max,  & 
                                                        &  T_min, inv_step, qint, gn, EkJ, Edis, una, ukb, fac_exp,   & 
                                                        &  Tmin_dis, Tmin_exc, nb_Da_proc, nb_exc_proc, da, exa
        ...
        REAL(KIND=8) :: tmp1, tmp2, tmp3, const1, const2, Shat_fac
        REAL(KIND=8) :: k0, a1, a2, c1, c2, delta1, delta2, sigma1, sigma2
        ...
        REAL(KIND=8), DIMENSION(:), ALLOCATABLE, SAVE :: kpf_KACHE
...
! Allocating vectors for Kache values
        ...
        IF (first_entry.EQ.0) THEN
           ...
           IF (da.EQV..TRUE.) THEN 
              ...
              ALLOCATE(kpf_KACHE(nb_Da_proc))
           ENDIF
           IF (exa.EQV..TRUE.) ALLOCATE(kef_KACHE(nb_exc_proc))
        ENDIF
        first_entry = 1
#endif
        ...
        IF (Tclip.NE.T_old) THEN
           ...
           IF (da.EQV..TRUE.) THEN
              ...
              DO i = 1,nb_Da_proc
                 kpf_KACHE(i) = ap_kf(i)*DEXP(bp_kf(i)*ln_T - cp_kf(i)*ov_T)
              ENDDO
           ENDIF
        ...
        IF (da.EQV..TRUE.) THEN
          ...
          DO i = 1,nb_bins 
             ...
             kfp = ap_kf(i)*DEXP(bp_kf(i)*ln_T - cp_kf(i)*ov_T)
#endif
      ...
      END SUBROUTINE source_BRVC

      !----------------------------------------------------!
      ! This subroutine computes the source term due to collisional processes for the N-N2 system.
      ! This subroutine is called when using the BRVC model.
      SUBROUTINE source_BRVC_Jac (rhoi, temp, omega, js_omega)

        USE mod_nitrogen_NASA_initialize_CFD,        ONLY: nb_ns, nb_bins, nb_eq, mm_N, mm_N2, ad_kf, bd_kf, cd_kf,  &  
                                                        &  ae_kf, be_kf, ce_kf, ap_kf, bp_kf, cp_kf, qint, dqintdT,  & 
                                                        &  T_store, T_max, T_min, inv_step, gn, EkJ, Edis, una, ukb, & 
                                                        &  fac_keq, Tmin_dis, Tmin_exc, da, exa
        USE mod_nitrogen_NASA_CFD_prop,              ONLY: Q_trans, Q_trans_der

        INTEGER :: i, ip, i1, ip1, pos1, pos2, pos3, pos4, posT
        INTEGER :: left, right
        REAL(KIND=8) :: ov_mmN, ov_mmN2
        ...
        DO i = 1,nb_bins 
           xi(i) = MAX(rhoi(i + 1)*ov_mmN2,xi_tol)
        ENDDO
        ...
        ! Bin internal partition functions (linear interpolation is used)
        DO i = 1,nb_bins
           tmp2 = qint(left + i)
           tmp3 = dqintdT(left + i)
           qk(i)     = tmp2 + tmp1*(qint(right + i) - tmp2)
        ...
        ! Source term and related Jacobian
        IF (T.GE.Tmin_dis) THEN
           ...
           IF (da.EQV..TRUE.) THEN
              ...
              DO i = 1,nb_bins 
                 ...
                 fac1 = (kf*tmp3 - 3.d0*kb*xns)*ov_mmN - 2.d0*kfp*xn*ov_keq*ov_mmN 
                 fac2 = xn*kf*ov_mmN2 
      ...
      END SUBROUTINE source_BRVC_Jac 

  END MODULE mod_nitrogen_NASA_CFD_source_BRVC
!------------------------------------------------------------------------------!
