!------------------------------------------------------------------------------!
! This module implements an interface for the Nitrogen_TTLTH thermodynamic library. It provides an
...
! Therefore it can plugged to any code without big efforts. 
  MODULE mod_Nitrogen_TTLTH_library 
...
#include"../config.h"
    ...
    REAL(KIND=8), ALLOCATABLE, DIMENSION(:) :: js_line  

    ! Subroutines of Nitrogen_TTLTH thermodynamic library interface. In some cases they call 
    ...
    CONTAINS
      ...
      SUBROUTINE Nitrogen_TTLTH_initialize (input_ns, input_ntrot, input_ntvib, input_nte, input_ntemp, input_neq,  & 
                                          & input_ndim, input_solver, input_mixture, input_reaction, state_model,   &
                                          & input_transf, input_path, input_xi_tol) 
  
        USE mod_nitrogen_TTLTH_initialize_CFD,            ONLY: initialize, create_tables, n_T
        USE mod_nitrogen_TTLTH_zeta,                      ONLY: set_pointer 
        ...
        IF (nt.NE.nb_temp) THEN
           PRINT*
           WRITE(*,5)input_solver(1:length),'::Nitrogen_TTLTH_library interface'   
           PRINT*
        ...
        ! Allocation of vector for source term Jacobian (fvmcc solver only)
        IF (input_solver(1:length).EQ.'solver_fvmcc_F90') THEN
           ALLOCATE(js_line((nb_ns + nb_temp)*(nb_ns + nb_temp - 1))) 
        ENDIF

        ! Nitrogen_TTLTH library is initialized
        ...
        ! creation of tables
!        call create_tables ( 700.d0,15000.d0,700.d0,15000.d0,700.d0,150000.d0,n_T )
!        call create_tables ( 700.d0,15000.d0,-1.d-3,1.d-3,-1.d-3,1.d-3,n_T )
        call create_tables ( 700.d0,15000.d0,-1.d-3,1.d-3,n_T )
        ...
        ! Useful indices 
        posTr = 1 + nb_trot
        posTv = posTr + 1!nb_tvib

5     FORMAT(A,A)

      END SUBROUTINE Nitrogen_TTLTH_initialize

      !------------------------------------------------------!
      ! This subroutine finalizes the Nitrogen TTLTH thermodynamic library
      SUBROUTINE Nitrogen_TTLTH_finalize ()

         USE mod_nitrogen_TTLTH_initialize_CFD,            ONLY: finalize
      ...
      END SUBROUTINE Nitrogen_TTLTH_finalize

      !------------------------------------------------------!
      ! This subroutine computes post-shock nonequilibrium conditions
      SUBROUTINE Nitrogen_TTLTH_post_shock_neq (p1, u1, T1, p2, u2, T2, yi, xN)

        USE mod_nitrogen_TTLTH_CFD_prop,                  ONLY: post_shock_neq
      ...
      END SUBROUTINE Nitrogen_TTLTH_post_shock_neq

      !------------------------------------------------------!
      ! This subroutine computes post-shock equilibrium conditions
      SUBROUTINE Nitrogen_TTLTH_post_shock_eq (p1, u1, T1, p2, u2, T2, yi, rhoi, xN)

        USE mod_nitrogen_TTLTH_CFD_eq,                  ONLY: post_shock_eq
      ...
      END SUBROUTINE Nitrogen_TTLTH_post_shock_eq

      !------------------------------------------------------!
      ! This subroutine computes common factors for the ODE system in 
      ! case of thermla nonequilibrium
      SUBROUTINE Nitrogen_TTLTH_compute_ODE_factors_tcneq (temp, omega, yi, fac_num, fac_den)

        USE mod_nitrogen_TTLTH_CFD_prop,            ONLY: ev_cvv_ho
      ...
      END SUBROUTINE Nitrogen_TTLTH_compute_ODE_factors_tcneq
      ...
      SUBROUTINE Nitrogen_TTLTH_write_ODE_solution (flow_file, pop_file, k, xold, x, uold, m_dot, z)
...
10    FORMAT(100E20.10)
      ...
      END SUBROUTINE Nitrogen_TTLTH_write_ODE_solution 

      !------------------------------------------------------!
      ! This subroutine writes down the solution when solving the flow equations 
      ! by means of Finite Volume method
      SUBROUTINE Nitrogen_TTLTH_write_fvmcc_solution (flow_file, pop_file, k, xold, x, uold, u, rhoi, temp)
        ...
        ! Density 
        CALL Nitrogen_TTLTH_get_density (rhoi, rho)

        ! Species mass fractions
        CALL Nitrogen_TTLTH_get_mass_fractions (rho, rhoi, yi)
...
10    FORMAT(100E20.10)
      ...
      END SUBROUTINE Nitrogen_TTLTH_write_fvmcc_solution   

      !------------------------------------------------------!
      ! This subroutine computes the equilibrium composition given pressure and temperature
      SUBROUTINE Nitrogen_TTLTH_compute_eq_composition (p, T, rhoi)

        USE mod_nitrogen_TTLTH_CFD_eq,                 ONLY: eq_composition 
      ...
      END SUBROUTINE Nitrogen_TTLTH_compute_eq_composition

      !------------------------------------------------------!
      ! This subroutine computes the N-N2 equilibrium internal energy per unit mass 
      SUBROUTINE Nitrogen_TTLTH_compute_eq_species_energy (T, e)

        USE mod_nitrogen_TTLTH_CFD_eq,           ONLY: energy
      ...
      END SUBROUTINE Nitrogen_TTLTH_compute_eq_species_energy 

      !------------------------------------------------------!
      ! This subroutine computes the N-N2 equilibrium enthalpy per unit mass 
      SUBROUTINE Nitrogen_TTLTH_compute_eq_species_enthalpy (T, h)

        USE mod_nitrogen_TTLTH_CFD_eq,           ONLY: enthalpy
      ...
      END SUBROUTINE Nitrogen_TTLTH_compute_eq_species_enthalpy 
      ...
      SUBROUTINE Nitrogen_TTLTH_compute_eq_species_entropy (p, T, yi, xi, smix, s)

        USE mod_nitrogen_TTLTH_CFD_eq,           ONLY: entropy
      ...
      END SUBROUTINE Nitrogen_TTLTH_compute_eq_species_entropy    

      !------------------------------------------------------!
      ! This subroutine computes the temperatures and thermodynamic data,
      ! additional data for Jacobians are given in output
      SUBROUTINE Nitrogen_TTLTH_get_data (rho, rhoi, rho_eint, temp, c, gamma, p, alpha, beta, ei)
 
        USE mod_function_pointer_TTLTH,      ONLY: get_temp_TTLTH, get_species_cv_TTLTH
        USE mod_nitrogen_TTLTH_CFD_prop,     ONLY: get_species_energy, ev_cvv_ho, get_energy
        ...
        REAL(KIND=8) :: cvr, cvv, er
        REAL(KIND=8) :: T, Tr, ki
        REAL(KIND=8), DIMENSION(nb_ns) :: cv  
	    real(kind=8), dimension(nb_tvib) :: Tv, evget, cvget
        ...
        CALL get_temp_TTLTH (rhoi, rho_eint, temp)
        T = temp(1)
        CALL get_species_cv_TTLTH (T, cv)
...
!print*, "Nitrogen_TTLTH_get_data ", 	  temp
...
!print*, "Nitrogen_TTLTH_get_data :: ", temp
        ...
        IF (nb_int_temp.GE.1) THEN
           ...
           Tr = temp(posTr) 
           Tv = temp(posTv:posTv+nb_tvib-1)

           !CALL ev_cvv_ho(Tv, ev, cvv)
		   call get_energy(Tv,evget,cvget)
!print*,"get_data ::", Tvl,Tvh
! corection : cvl = de/dT = de/daT daT/dT avec daT/dT = aT**2           
		   cvr = Ri(2)
           er  = cvr*Tr

           pos = MAX(2,posTr)
           beta(pos)   = sum(rhoi(2:1+nb_tvib))*cvr
           do is=1,nb_tvib
				beta(posTv+is-1) = rhoi(1+is)*cvget(is)
		   enddo

           ! Nonequilibrium energy submatrix ei = [eN eN2l eN2h eN_T1 eN_T2 eN2l_T1 ...]
           ! Nitrogen atom N 
           DO is = 1,nb_int_temp 
              ei(nb_ns + is) = 0.d0 
           ENDDO
 
           ! Nitrogen molecule N2 low
		   do is=1, nb_tvib
				ei(nb_ns + is*nb_int_temp + pos   - 1) = er
				ei(nb_ns + is*nb_int_temp + posTv - 1 : nb_ns + (is+1)*nb_int_temp - 1 ) = 0.d0
				ei(nb_ns + is*nb_int_temp + posTv + is - 2) = evget(is)
		   enddo

        ENDIF
  
      END SUBROUTINE Nitrogen_TTLTH_get_data 
  
      !------------------------------------------------------!
      ! This subroutine computes thermodynamic data needed to be stored
      ! for nonequilibrium gas flow computations. 
      SUBROUTINE Nitrogen_TTLTH_get_thermodynamic_data (rho, rhoi, temp, c, gamma, p, alpha, beta, ei)
  
        USE mod_function_pointer_TTLTH,      ONLY: get_species_cv_TTLTH
        USE mod_nitrogen_TTLTH_CFD_prop,     ONLY: get_species_energy, ev_cvv_ho , get_energy
        ...
        REAL(KIND=8) :: tmp
        REAL(KIND=8) :: sum1, sum2
        REAL(KIND=8) :: cvr, cvv, er!, evl, evh, cvl, cvh
        REAL(KIND=8) :: T, Tr,  ki !, Tvl, Tvh
        REAL(KIND=8), DIMENSION(nb_ns) :: cv  
        ...
        IF (nb_int_temp.GE.1) THEN
           ...
           ! Nitrogen molecule N2 high
           !ei(nb_ns + 2*nb_int_temp + pos   - 1) = er
           !ei(nb_ns + 2*nb_int_temp + posTv - 1) = 0.d0
		   !ei(nb_ns + 2*nb_int_temp + posTv) = evh

        ENDIF
  

      END SUBROUTINE Nitrogen_TTLTH_get_thermodynamic_data
 
      !------------------------------------------------------!
      ! This subroutine computes the energy density for total internal,
      ! rotational, vibrational and electronic components.
      SUBROUTINE Nitrogen_TTLTH_get_energy_densities (rhoi, temp, rho_e)
  
        USE mod_nitrogen_TTLTH_CFD_prop,    ONLY: energy  
  
        INTEGER :: is
        REAL(KIND=8) :: er, tmp
        REAL(KIND=8), DIMENSION(nb_ns) :: eint
		real(kind=8), dimension(nb_tvib) :: ev
  
        REAL(KIND=8), DIMENSION(:), INTENT(IN) :: rhoi, temp
        REAL(KIND=8), INTENT(OUT), DIMENSION(:) :: rho_e
! print*, "clique", temp, ev       
		
        CALL energy (temp, er, ev, eint)
   
        rho_e(posTr) = sum(rhoi(2:1+nb_tvib))*er
        do is = 1, nb_tvib
			rho_e(posTv+is-1) = rhoi(1+is)*ev(is)
		enddo
!   print*, "Nitrogen_TTLTH_get_energy_densities :: ", temp(2), ev, rhoi(2)    
      ...
      SUBROUTINE Nitrogen_TTLTH_get_energy (rho, rhoi, temp, e)
        ...
        e(posTr) = sum(yi(2:1 + nb_tvib))*er
        DO is = 1,nb_tvib
           e(posTv + is - 1) = yi(1 + is)*ev(is)
        ENDDO
      ...
      END SUBROUTINE Nitrogen_TTLTH_get_energy
  
      !------------------------------------------------------!
      ! This subroutine computes the total internal, rotational, vibrational and electronic enthalpy 
      ! per unit mass of the mixture.
      SUBROUTINE Nitrogen_TTLTH_get_enthalpy (rho, rhoi, temp, h)
  
        USE mod_nitrogen_TTLTH_CFD_prop,    ONLY: enthalpy  
  
        INTEGER :: is
        REAL(KIND=8) :: er, tmp !, evl, evh
        ...
        !tmp = yi(2)
        h(posTr) = sum(yi(2:1+nb_tvib))*er 
        do is = 1, nb_tvib
			h(posTv+is-1) = yi(1+is)*ev(is)
		enddo
      ...
      END SUBROUTINE Nitrogen_TTLTH_get_enthalpy 
 
      !----------------------------------------------------!
      ! This subroutine computes the species internal energies and constant 
      ! volume specific heat per unit mass
      SUBROUTINE Nitrogen_TTLTH_get_species_energy (temp, e) 
 
        USE mod_Nitrogen_TTLTH_CFD_prop,    ONLY: get_species_energy
      ...
      END SUBROUTINE Nitrogen_TTLTH_get_species_energy

      !----------------------------------------------------!
      ! This subroutine computes the species internal energies heat per unit mass
      SUBROUTINE Nitrogen_TTLTH_get_species_energy_cv (temp, e, cv) 
 
        USE mod_Nitrogen_TTLTH_CFD_prop,    ONLY: get_species_energy_cv
      ...
      END SUBROUTINE Nitrogen_TTLTH_get_species_energy_cv

      !----------------------------------------------------!
      ! This subroutine computes the species internal energies and constant 
      ! volume specific heat per unit mass
      SUBROUTINE Nitrogen_TTLTH_get_species_enthalpy (temp, h) 
      ...
      END SUBROUTINE Nitrogen_TTLTH_get_species_enthalpy

      !----------------------------------------------------!
      ! This subroutine computes the species enthalpies and constant 
      ! pressure specific heat per unit mass
      SUBROUTINE Nitrogen_TTLTH_get_species_enthalpy_cp (temp, h, cp) 
      ...
      END SUBROUTINE Nitrogen_TTLTH_get_species_enthalpy_cp

      !------------------------------------------------------!
      ! This subroutine computes the frozen specific heats at constant volume of each species
      SUBROUTINE Nitrogen_TTLTH_get_species_frozen_cv (T, cv)  
      ...
      END SUBROUTINE Nitrogen_TTLTH_get_species_frozen_cv 
  
      !------------------------------------------------------!
      ! This subroutine computes the frozen specific heats at constant pressure of each species
      SUBROUTINE Nitrogen_TTLTH_get_species_frozen_cp (T, cp)  
        ...
        CALL get_species_cv_TTLTH (T, cp)

        DO i = 1,nb_ns 
           cp(i) = cp(i) + Ri(i)
        ENDDO 

      END SUBROUTINE Nitrogen_TTLTH_get_species_frozen_cp
      ...
      SUBROUTINE Nitrogen_TTLTH_get_species_frozen_energy_cv (T, cv, e)  
 
        USE mod_function_pointer_TTLTH,      ONLY: get_species_energy_cv_TTLTH   
 
        REAL(KIND=8), INTENT(IN) :: T 
        REAL(KIND=8), DIMENSION(:), INTENT(OUT) :: cv, e
  
        CALL get_species_energy_cv_TTLTH (T, cv, e)

      END SUBROUTINE Nitrogen_TTLTH_get_species_frozen_energy_cv 
      ...
      SUBROUTINE Nitrogen_TTLTH_get_species_frozen_enthalpy_cp (T, cp, h)  
        ...
        CALL get_species_energy_cv_TTLTH (T, cp, h)
      ...
      END SUBROUTINE Nitrogen_TTLTH_get_species_frozen_enthalpy_cp

      !------------------------------------------------------!
      ! This subroutine computes the temperatures given the energy densities.
      SUBROUTINE Nitrogen_TTLTH_get_temperatures (rhoi, rho_eint, temp)
  
        USE mod_function_pointer_TTLTH,    ONLY: get_temp_TTLTH
      ...
      END SUBROUTINE Nitrogen_TTLTH_get_temperatures
 
      !------------------------------------------------------!
      ! This subroutine computes the equilibrium speed of sound
      SUBROUTINE Nitrogen_TTLTH_get_eq_sound_speed (rhoi, p, T, c)
        ...
        CALL Nitrogen_TTLTH_compute_eq_composition (p_pert, T, dens)
        CALL Nitrogen_TTLTH_get_density (dens, rho_pert)
        ...
        CALL Nitrogen_TTLTH_compute_eq_composition (p, T_pert, dens)
        CALL Nitrogen_TTLTH_get_density (dens, rho_pert)
        CALL Nitrogen_TTLTH_get_mass_fractions (rho_pert, dens, yi)
      ...
      END SUBROUTINE Nitrogen_TTLTH_get_eq_sound_speed 

      !------------------------------------------------------!
      ! This subroutine computes the equilibrium specific heat ratio
      SUBROUTINE Nitrogen_TTLTH_get_eq_gamma (rhoi, p, T, gamma)
      ...
      END SUBROUTINE Nitrogen_TTLTH_get_eq_gamma 

      !------------------------------------------------------!
      ! This subroutine computes the equilibrium specific heat ratio
      SUBROUTINE Nitrogen_TTLTH_get_eq_gamma_sound_speed (rhoi, p, T, gamma, c)
      ...
      END SUBROUTINE Nitrogen_TTLTH_get_eq_gamma_sound_speed 

      !------------------------------------------------------!
      ! This subroutine computes the frozen speed of sound
      SUBROUTINE Nitrogen_TTLTH_get_frozen_sound_speed (rhoi, temp, c)
      ...
      END SUBROUTINE Nitrogen_TTLTH_get_frozen_sound_speed
 
      !------------------------------------------------------!
      ! This subroutine gets the frozen speed specific heat ratio.
      SUBROUTINE Nitrogen_TTLTH_get_frozen_gamma (rhoi, temp, gamma)
      ...
      END SUBROUTINE Nitrogen_TTLTH_get_frozen_gamma
 
      !------------------------------------------------------!
      ! This subroutine gets the frozen speed of sound and specific heat ratio.
      SUBROUTINE Nitrogen_TTLTH_get_frozen_gamma_sound_speed (rhoi, temp, gamma, c)
      ...
      END SUBROUTINE Nitrogen_TTLTH_get_frozen_gamma_sound_speed
  
      !------------------------------------------------------!
      ! This subroutine computes the mixture static pressure (presence of free electrons is accounted for)
      SUBROUTINE Nitrogen_TTLTH_get_pressure (rhoi, temp, p)
      ...
      END SUBROUTINE Nitrogen_TTLTH_get_pressure
  
      !------------------------------------------------------!
      ! This subroutine computes the mixture density.
      SUBROUTINE Nitrogen_TTLTH_get_density (rhoi, rho)
      ...
      END SUBROUTINE Nitrogen_TTLTH_get_density
  
      !------------------------------------------------------!
      ! This subroutine computes the specific gas constant R.
      SUBROUTINE Nitrogen_TTLTH_get_Rgas (rhoi, R)
      ...
      END SUBROUTINE Nitrogen_TTLTH_get_Rgas
  
      !------------------------------------------------------!
      ! This subroutine gives the species gas constants
      SUBROUTINE Nitrogen_TTLTH_get_Ri (out_Ri)
      ...
      END SUBROUTINE Nitrogen_TTLTH_get_Ri 
 
      !------------------------------------------------------!
      ! This subroutine gives the species molar masses
      SUBROUTINE Nitrogen_TTLTH_get_mi (out_mi)
      ...
      END SUBROUTINE Nitrogen_TTLTH_get_mi
 
      !------------------------------------------------------!
      ! This subroutine computes the mixture number density 
      SUBROUTINE Nitrogen_TTLTH_get_mix_numb_dens (p, T, Te, xi, nb) 
      ...
      END SUBROUTINE Nitrogen_TTLTH_get_mix_numb_dens 

      !------------------------------------------------------!
      ! This subroutine computes the species mass fraction
      SUBROUTINE Nitrogen_TTLTH_get_mass_fractions (rho, rhoi, yi)
      ...
      END SUBROUTINE Nitrogen_TTLTH_get_mass_fractions
 
      !------------------------------------------------------!
      ! This subroutine computes the species molar fractions
      SUBROUTINE Nitrogen_TTLTH_get_molar_fractions (rhoi, xi)
      ...
      END SUBROUTINE Nitrogen_TTLTH_get_molar_fractions
      ...
      SUBROUTINE Nitrogen_TTLTH_molar_frac_from_mass_frac (yi, mm, xi)
      ...
      END SUBROUTINE Nitrogen_TTLTH_molar_frac_from_mass_frac 
     
      !------------------------------------------------------!
      ! This subroutine computes the mass fractions from molar fractions. 
      ! The mixture molar mass is also computed and given in output.
      SUBROUTINE Nitrogen_TTLTH_mass_frac_from_molar_frac (xi, mm, yi)
      ...
      END SUBROUTINE Nitrogen_TTLTH_mass_frac_from_molar_frac 
      ...
      SUBROUTINE Nitrogen_TTLTH_comp_tol(xi)
      ...
      END SUBROUTINE Nitrogen_TTLTH_comp_tol 
 
      !------------------------------------------------------!
      ! This subroutine computes the source term due to collisional and radiative processes.
      SUBROUTINE Nitrogen_TTLTH_get_source (rhoi, temp, s) 
  
        USE mod_nitrogen_TTLTH_CFD_source,         ONLY: source
  
        REAL(KIND=8), DIMENSION(:), INTENT(IN) :: rhoi, temp
        REAL(KIND=8), DIMENSION(:), INTENT(OUT) :: s
  
        CALL source (rhoi, temp, s)        
  
      END SUBROUTINE Nitrogen_TTLTH_get_source 
  
      !------------------------------------------------------!
      ! This subroutine computes the source term due to collisional and radiative processes 
      ! and its Jacobian.
      SUBROUTINE Nitrogen_TTLTH_get_source_Jac (rhoi, temp, s, js)   
  
        USE mod_nitrogen_TTLTH_CFD_source,         ONLY: source_Jac
 
        REAL(KIND=8), DIMENSION(:), INTENT(IN) :: rhoi, temp
        REAL(KIND=8), DIMENSION(:), INTENT(OUT) :: s, js

        ! Source term and source term Jacobian (with respect to primitive variables)
        CALL source_Jac (rhoi, temp, s, js)

      END SUBROUTINE Nitrogen_TTLTH_get_source_Jac

      !------------------------------------------------------!
      ! This subroutine computes the species diffusion velocities 
      SUBROUTINE Nitrogen_TTLTH_compute_species_DiffFlux (p, T, Te, xi, diff_driv, Ji) 
        ...
        PRINT*,'in "Nitrogen_TTLTH_compute_species_DiffFlux", not implemented yet..'
        STOP 

      END SUBROUTINE Nitrogen_TTLTH_compute_species_DiffFlux
  
      !------------------------------------------------------!
      ! This subrouotine computes the transport coefficients 
      SUBROUTINE Nitrogen_TTLTH_compute_transpCoeff (p, xi, temp, mu, kappa, lambda, Di, chi) 
        ...
        PRINT*,'in "Nitrogen_TTLTH_compute_transpCoeff", not implemented yet..'
        STOP 

      END SUBROUTINE Nitrogen_TTLTH_compute_transpCoeff 
#endif
  
  END MODULE mod_Nitrogen_TTLTH_library
!------------------------------------------------------------------------------!  
