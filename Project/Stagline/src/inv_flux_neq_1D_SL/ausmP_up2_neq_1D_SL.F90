...
!! for 1D stagnation line chemically non equilibrium flows.
  SUBROUTINE ausmP_up2_neq_1D_SL (nx, vol_l, vol_r, left_data, right_data, u_left, u_right, f)
    USE mod_general_data,                 ONLY: nb_eq, nb_ns, nb_int_temp, nb_te, &
                                              & pos_u_cell, pos_v_cell, pos_c_cell, pos_pres_cell, pos_h0_cell, &
                                              & pos_mu_cell, pos_rho, pos_rhou, pos_rhov, pos_rho_cell, pos_rhoE, &
                                              & pos_gamma_cell, MachIn
    ...
    INTEGER      :: i_ns, i_int_temp

    REAL(KIND=8) :: Ma_function_minus_4, Ma_function_plus_4, P_function_minus_5, P_function_plus_5
    REAL(KIND=8) :: Ma_p_diffusion, P_u_diffusion_slau2, compute_a_interface
    ...
    a_interface = compute_a_interface(unl, unr, h0l, h0r, gamma_interface)
    ...
    Pu = P_u_diffusion_slau2( rhol, rhor, unl, unr, MnL, MnR, a_interface, K_u, alpha_coeff ) 
    ...
    P_interface = ((PpL - Pmr)*(pl-pr) + pl + pr)/2.0D0 + Pu
    ...
    Psil(pos_rhov) = vl
    Psil(pos_rhoE) = h0l
    DO i_int_temp = 1, nb_int_temp + nb_te
      Psil(pos_rhoE+i_int_temp) = u_left(pos_rhoE+i_int_temp) / rhol        
    ENDDO
    ...
    Psir(pos_rhoE) = h0r
    DO i_int_temp = 1, nb_int_temp+nb_te
      Psir(pos_rhoE+i_int_temp) = u_right(pos_rhoE+i_int_temp) / rhor
    ENDDO
  ...
  END SUBROUTINE ausmP_up2_neq_1D_SL
...
FUNCTION P_u_diffusion_slau2 ( rhoL, rhoR, uL, uR, MaL, MaR, alpha_interface, K_u, alpha_coeff )
IMPLICIT NONE
...
REAL(KIND=8)             :: P_u_diffusion_slau2

 P_u_diffusion_slau2 = SQRT(0.50D0*(uL*uL+uR*uR))*(P_function_plus_5(MaL, alpha_coeff)+ P_function_minus_5(MaR, alpha_coeff)-1.D0)*&
                             0.5D0*(rhoL+rhoR)*alpha_interface
END FUNCTION P_u_diffusion_slau2

!!------------------------------------------------------------------------------!
!
!FUNCTION compute_a_interface(unl, unr, h0l, h0r, gamma_interface)
!IMPLICIT NONE
!REAL(KIND=8), INTENT(IN) :: unl, unr, h0l, h0r
!REAL(KIND=8), INTENT(IN) :: gamma_interface
!REAL(KIND=8)             :: a_star_2, a_star, a_hat_L, a_hat_R
!REAL(KIND=8)             :: compute_a_interface
!
...
!
!compute_a_interface = MIN(a_hat_L, a_hat_R)
!
!END FUNCTION compute_a_interface
!
!!------------------------------------------------------------------------------!
!
