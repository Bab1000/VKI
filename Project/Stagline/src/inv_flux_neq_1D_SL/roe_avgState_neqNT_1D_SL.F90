...
!! In this case the flow is characterized by N temperatures. However, no separated temperature exist for the free electrons.
  SUBROUTINE roe_avgState_neqNT_1D_SL (u_left, u_right, left_data, right_data, u_Roe, data_Roe)
    ...
    USE mod_general_data,             ONLY: nb_ns, nb_eq, nb_temp, pos_u_cell, pos_v_cell, pos_h0_cell, &
                                          ...
                                          & beta, temp, nb_int_temp, pos_rhoek
    ...
    ALLOCATE(Tkl(nb_int_temp), Tkr(nb_int_temp), Tk(nb_int_temp), rhoek(nb_int_temp))
    ...
    DO i = 1,nb_ns*(nb_int_temp+1)
       data_Roe(pos_ei_cell + i - 1) = ei(i)
    ENDDO
    
  END SUBROUTINE roe_avgState_neqNT_1D_SL
!------------------------------------------------------------------------------!
