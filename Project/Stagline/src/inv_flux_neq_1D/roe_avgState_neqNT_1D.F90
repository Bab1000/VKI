...
!! In this case the flow is characterized by N temperatures. However, no separated temperature exist for the free electrons.
  SUBROUTINE roe_avgState_neqNT_1D (u_left, u_right, left_data, right_data, u_Roe, data_Roe)
    ...
    USE mod_general_data,             ONLY: nb_ns, nb_eq, nb_temp, nb_int_temp, pos_u_cell, pos_h0_cell, &
                                          ...
                                          & pos_beta_cell, pos_rhou, pos_rhoE, pos_rhoek, yi, rhoi,      & 
                                          & ei, beta, temp
    ...
    REAL(KIND=8) :: c, c2, cn, ek, h0, rho, rhou, rhoE, p, u
    ...
    ! Average density, total enthalpy, temperatures, velocity and kinetic energy
  ...
  END SUBROUTINE roe_avgState_neqNT_1D 
!------------------------------------------------------------------------------!
