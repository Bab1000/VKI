...
!! (no separated equations for free electrons). 
  SUBROUTINE Steger_Warming_neqNT_1D (nx, left_data, right_data, u_left, u_right, f) 

    USE mod_general_data,             ONLY: nb_ns, nb_int_temp, nb_te, pos_u_cell, pos_rho_cell, & 
                                          & pos_c_cell, pos_h0_cell, pos_gamma_cell, pos_rhou,   & 
                                          & pos_rhoE, yi
    ...
    fac = fac/rho
    DO i = 1,nb_int_temp
       fl(pos_rhoE + i) = fac*u_left(pos_rhoE + i)
    ENDDO
    ...
    fac = fac/rho
    DO i = 1,nb_int_temp
       fr(pos_rhoE + i) = fac*u_right(pos_rhoE + i)
    ENDDO

    ! Sum of positive and negative split fluxes (F = F^+ + F^-)
    f = 0.5d0*(fac_l*fl + fac_r*fr)

  END SUBROUTINE Steger_Warming_neqNT_1D  
!------------------------------------------------------------------------------!
