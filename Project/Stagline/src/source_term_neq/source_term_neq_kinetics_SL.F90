...
!! processes for nonequilibrium flows.
  SUBROUTINE source_term_neq_kinetics_SL (cell_id, r, cell_data, cons, s)
  ...
  END SUBROUTINE source_term_neq_kinetics_SL
!------------------------------------------------------------------------------!
!> This subroutine provides the source terms due to collisional and radiative
...
!! in the free-electron pseudo-entropy equation. 
  SUBROUTINE source_term_neq_el_kinetics_SL (cell_id, r, cell_data, cons, s)
  ...
  END SUBROUTINE source_term_neq_el_kinetics_SL
!------------------------------------------------------------------------------!
!> This subroutine provides the source term and the related Jacobian due to collisional and radiative
!! processes for nonequilibrium stagnation line flows.
  SUBROUTINE source_term_neq_kinetics_Jac_SL (source_id, cell_id, r, cell_data, cons, s, js)

    USE mod_general_data,            ONLY: nb_ns, nb_dim, nb_temp, nb_int_temp, nb_eq, pos_u_cell, pos_ek_cell,  & 
                                         & pos_beta_cell, pos_betak_cell, pos_ei_cell, pos_eki_cell, pos_T_cell, & 
                                         & pos_u_cell, pos_rhou, pos_rhov, pos_rhoE, pos_rhoek, rhoi, temp, eiT, & 
                                         & ov_betak
    ...
    INTEGER :: i, j, l, k
    INTEGER :: dim_js, ns_p1, pos, pos_k
    REAL(KIND=8) :: fac, tmp, tmp1, tmp2
    REAL(KIND=8) :: beta, inv_beta, ek, u
    ...
    ! Radial velocity component and kinetic energy per unit mass
    ...
    DO j = 1,nb_ns 
       ...
       ! Radial and circumferential momenum equations
       js(pos_rhou,j) = 0.d0 
       js(pos_rhov,j) = 0.d0 
    ...
    tmp1 = - u*inv_beta
    DO i = 1,nb_ns 
       js(i,pos_rhou) = tmp1*js_line((i - 1)*dim_js + ns_p1)
    ENDDO

    ! Radial and circumferential momenum equations
    js(pos_rhou,pos_rhou) = 0.d0
    js(pos_rhov,pos_rhou) = 0.d0 

    ! Global energy equation
    js(pos_rhoE,pos_rhou) = 0.d0

    ! Internal energy equations
    DO k = 1,nb_int_temp 
       js(pos_rhoek + k - 1,pos_rhou) = tmp1*js_line((nb_ns + k - 1)*dim_js + ns_p1)
    ENDDO
    ...
    DO k = 1,nb_int_temp 
       ...
       ! Radial and circumferential momenum equations
       js(pos_rhou,pos_k) = 0.d0
       js(pos_rhov,pos_k) = 0.d0
  ...
  END SUBROUTINE source_term_neq_kinetics_Jac_SL
!------------------------------------------------------------------------------!
!> This subroutine provides the source term and the related Jacobian due to collisional and radiative
!! processes for nonequilibrium flows.
...
!! of a separate free electron pseudo-entropy equation
  SUBROUTINE source_term_neq_el_kinetics_Jac_SL (source_id, cell_id, r, cell_data, cons, s, js)
    ...
    PRINT*,'in "source_term_neq_el_kinetics_Jac_SL", to be implemented!'
    STOP

  END SUBROUTINE source_term_neq_el_kinetics_Jac_SL
!------------------------------------------------------------------------------!
