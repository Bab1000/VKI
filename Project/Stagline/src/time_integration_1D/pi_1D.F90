!------------------------------------------------------------------------------!
!> This subroutine computes the solution variation in case of use of a point implicit time-integration scheme for 1D flows.
  SUBROUTINE pi_1D ()
...
#include "../config.h"
    ...
    USE mod_function_pointer,        ONLY: conv_flux_1D, diff_flux_1D, rec_1D, compute_time_step_1D,                 & 
                                         & get_source_term_Jac, get_visc_spectral_radius_1D, get_inv_spectral_radius_1D
#ifdef GOTO_BLAS 
    ...
    REAL(KIND=8) :: lambda_inv, lambda_visc
    ...
    DO i = 1,nb_cells
       ...
       ! Spectral radius for flux Jacobian approximation
       ! Inviscid spectral radius
       CALL get_inv_spectral_radius_1D (prop_l, lambda_inv)

       ! Viscous spectral radius 
       CALL get_visc_spectral_radius_1D (prop_l, lambda_visc) 

       ! Lhs matrix (inclusion of the source term Jacobian(s) and spectral radii of inviscid and diffusive flux Jacobians)
       js = iden*(vol_ov_dt + lambda_inv + 4.d0*lambda_visc/(vol_l + vol_r)) - js*vol_l
