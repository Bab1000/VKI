!------------------------------------------------------------------------------!
!> This subrotine provides the numerical flux Jacobian for 1D calorically perfect gas flows
!! according to the positive and negative split of the inviscid flux Jacobian.
  SUBROUTINE Pos_Neg_split_Jac_1D (nx, left_data, right_data, u_left, u_right, jfl, jfr)

    USE mod_general_data,               ONLY: nb_eq, pos_u_cell, pos_c_cell, pos_ek_cell, pos_h0_cell, gamma

    IMPLICIT NONE

    INTEGER :: i, j
    REAL(KIND=8) :: c, ek, h0, nx_ov_c, ov_c, ov_c2, u, v2, vn, vn_ov_c
    ...
    jfl(3,1) = l1p_ek + (eig_sum*h0 - l1p_ek)*gamma_minus1_ov_c2_ek - &
    ...
    jfl(3,2) = l1p*gamma_minus1_ov_c2_ek_u + eig_sum*(vn*nx - gamma_minus1_ov_c2_u*h0) + &
             & eig_diff*(h0*nx_ov_c - gamma_minus1_ov_c*u*vn)

    ! Third column 
    jfl(1,3) = eig_sum_m_l1p*gamma_minus1_ov_c2
    jfl(2,3) = eig_sum_m_l1p*gamma_minus1_ov_c2_u + eig_diff*gamma_minus1*nx_ov_c
    jfl(3,3) = - l1p*gamma_minus1_ov_c2_ek + eig_sum*gamma_minus1_ov_c2*h0 + & 
    ...
    jfr(2,1) = l1m*u + eig_sum_m_l1m*gamma_minus1_ov_c2_ek_u - eig_sum*nx*vn + & 
             & eig_diff*(gamma_minus1_ov_c*ek*nx - u*vn_ov_c)
    jfr(3,1) = l1m_ek + (eig_sum*h0 - l1m_ek)*gamma_minus1_ov_c2_ek - &
             & eig_sum*vn**2 + eig_diff*(gamma_minus1_ov_c*ek*vn - h0*vn_ov_c) 
    ...
    jfr(2,2) = l1m*gamma_minus1_ov_c2_v2 + eig_sum*(nx2 - gamma_minus1_ov_c2_v2) + & 
             & eig_diff*t_m_gamma*vn_ov_c
    jfr(3,2) = l1m*gamma_minus1_ov_c2_ek_u + eig_sum*(vn*nx - gamma_minus1_ov_c2_u*h0) + &
             & eig_diff*(h0*nx_ov_c - gamma_minus1_ov_c*u*vn)

    ! Third column 
    jfr(1,3) = eig_sum_m_l1m*gamma_minus1_ov_c2
    jfr(2,3) = eig_sum_m_l1m*gamma_minus1_ov_c2_u + eig_diff*gamma_minus1*nx_ov_c
    jfr(3,3) = - l1m*gamma_minus1_ov_c2_ek + eig_sum*gamma_minus1_ov_c2*h0 + & 
             & eig_diff*gamma_minus1_ov_c*vn

  END SUBROUTINE Pos_Neg_split_Jac_1D
!------------------------------------------------------------------------------!
