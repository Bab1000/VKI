...
!! according to the approximation proposed by Yoon and Jameson.
  SUBROUTINE Yoon_Jameson_Jac_1D (nx, left_data, right_data, u_left, u_right, jfl, jfr)
    ...
    ! Explicit interface for external subroutine inviscid_flux_Jac_1D
    INTERFACE 
      SUBROUTINE inviscid_flux_Jac_1D (nx, phys_data, a) 
        REAL(KIND=8), INTENT(IN) :: nx
        REAL(KIND=8), DIMENSION(:), INTENT(IN) :: phys_data
        REAL(KIND=8), DIMENSION(:,:), INTENT(OUT) :: a
      END SUBROUTINE inviscid_flux_Jac_1D
    END INTERFACE

    ! Left and right states data
    ...
    IF (Mnl.GE.1.d0) THEN
       ...
       ! Supersonic flow
       CALL inviscid_flux_Jac_1D (nx, left_data, jfl) 
    ...
    IF (Mnr.LE.-1.d0) THEN
       ...
       ! Supersonic flow
       CALL inviscid_flux_Jac_1D (nx, right_data, jfr) 
  ...
  END SUBROUTINE Yoon_Jameson_Jac_1D
!------------------------------------------------------------------------------!
