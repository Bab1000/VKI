...
!! calorically perfect gas flows when solving the Euler equations. 
  SUBROUTINE cons_to_phys_1D_SL_Eu (cons, phys_prop)

    USE mod_general_data,        ONLY: pos_u_cell, pos_v_cell, pos_h0_cell, pos_c_cell, pos_ek_cell,  & 
                                     & pos_pres_cell, pos_rho_cell, pos_T_cell, gamma, R_gas

    IMPLICIT NONE

    REAL(KIND=8) :: rho, rhoE
    REAL(KIND=8) :: ov_rho, rhoE_ov_rho, p_ov_rho
    REAL(KIND=8) :: c, ek, p, u, v
  ...
  END SUBROUTINE cons_to_phys_1D_SL_Eu
!------------------------------------------------------------------------------!
!> This subroutine computes the physical properties from conservative variables for 1D stagnation line
!! calorically perfect gas flows when solving the Navier-Stokes equations. 
  SUBROUTINE cons_to_phys_1D_SL_Ns (cons, phys_prop)

    USE mod_general_data,        ONLY: pos_u_cell, pos_v_cell, pos_h0_cell, pos_c_cell, pos_ek_cell,  & 
                                     & pos_pres_cell, pos_rho_cell, pos_T_cell, pos_mu_cell,          & 
                                     & pos_kappa_cell, pos_lambda_cell, R_gas, gamma
    ...
    REAL(KIND=8) :: rho, rhoE
    REAL(KIND=8) :: ov_rho, rhoE_ov_rho, p_ov_rho
    REAL(KIND=8) :: c, lambda, ek, mu, p, T, u, v
  ...
  END SUBROUTINE cons_to_phys_1D_SL_Ns
!------------------------------------------------------------------------------!
