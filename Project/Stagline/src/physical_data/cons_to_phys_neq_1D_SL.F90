!------------------------------------------------------------------------------!
!> This subroutine computes the physical properties from conservative variables for 1D stagnation line
!! nonequilibrium gas flows when solving the Euler equations. 
  SUBROUTINE cons_to_phys_neq_1D_SL_Eu (cons, phys_prop)

    USE mod_general_data,            ONLY: nb_ns, nb_temp, nb_int_temp, pos_rhou, pos_rhov, pos_rhoE,  &
                                         & pos_u_cell, pos_v_cell, pos_h0_cell, pos_c_cell,            & 
                                         & pos_gamma_cell, pos_pres_cell, pos_rho_cell, pos_ek_cell,   & 
                                         & pos_alpha_cell, pos_beta_cell, pos_ei_cell, pos_T_cell,     & 
                                         & rho_eint, rhoi, temp, ei, beta, nb_te
    ...
    INTEGER :: i, k
    REAL(KIND=8) :: tmp
    REAL(KIND=8) :: rho, rhoE, ov_rho
    REAL(KIND=8) :: alpha, c, gamma, ek, p, u, v
    ...
    phys_prop(pos_h0_cell)    = (rhoE + p)*ov_rho
    ...
    DO i = 1,nb_int_temp + nb_te + 1
       phys_prop(pos_beta_cell + i - 1)  = beta(i) 
    ENDDO

    DO i = 1,nb_ns + nb_ns*(nb_int_temp+nb_te)
       phys_prop(pos_ei_cell + i - 1) = ei(i)  
    ENDDO

  END SUBROUTINE cons_to_phys_neq_1D_SL_Eu
!------------------------------------------------------------------------------!
!> This subroutine computes the physical properties from conservative variables for 1D stagnation line 
!! nonequilibrium gas flows when solving the Navier-Stokes equations. 
  SUBROUTINE cons_to_phys_neq_1D_SL_Ns (cons, phys_prop)

    USE mod_general_data,            ONLY: nb_ns, nb_temp, nb_int_temp, pos_rhou, pos_rhov, pos_rhoE,  & 
                                         & pos_u_cell, pos_v_cell, pos_h0_cell, pos_c_cell,            & 
                                         & pos_gamma_cell, pos_pres_cell, pos_rho_cell, pos_ek_cell,   & 
                                         & pos_alpha_cell, pos_beta_cell, pos_ei_cell, pos_mu_cell,    & 
                                         & pos_kappa_cell, pos_lambda_cell, pos_T_cell, pos_Di_cell,   & 
                                         & pos_chi_cell, rho_eint, rhoi, temp, ei, Di, chi, beta,      & 
                                         & xi, lambda_vec, nb_te
    ...
    REAL(KIND=8) :: tmp
    REAL(KIND=8) :: rho, rhoE, ov_rho
    REAL(KIND=8) :: alpha, c, gamma, ek, kappa, mu, p, u, v
  ...
  END SUBROUTINE cons_to_phys_neq_1D_SL_Ns
!------------------------------------------------------------------------------!
