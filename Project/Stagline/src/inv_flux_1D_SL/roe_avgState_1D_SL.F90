!------------------------------------------------------------------------------!
!> This subroutine computes the Roe's averaged state for 1D stagnation line calorically perfect gas flows.
  SUBROUTINE roe_avgState_1D_SL (left_data, right_data, roe_data)

    USE mod_general_data,     ONLY: nb_eq, pos_u_cell, pos_v_cell, pos_pres_cell, pos_h0_cell, pos_c_cell, & 
    ...
    REAL(KIND=8) :: tmp
    REAL(KIND=8) :: a, b, r
    REAL(KIND=8) :: c, ek, h0, u, v, rho, p
    REAL(KIND=8) :: h0l, h0r, rhol, rhor, ul, ur, vl, vr
    ...
    ! Velocity components, total enthalpy and density of left and right states
