...
! when using the TTLTH multi-temperature model.
  MODULE mod_nitrogen_TTLTH_CFD_prop

  USE mod_nitrogen_TTLTH_initialize_CFD,         ONLY: posTr, posTv   
  ...
  CONTAINS
    ...
    SUBROUTINE Q_int_rr_ho (T, Q)
      ...
      USE mod_nitrogen_TTLTH_initialize_CFD,     ONLY: theta_vib, theta_rot, EkJ, ukb
	  ...
	  Q = T/(theta_rot*2.d0) * sum(   dexp( - EkJ/(ukb*T) )   )
    ...
    SUBROUTINE Q_int_rr_ho_der (T, Q, dQint_dT)
      ...
      REAL(KIND=8) :: fac_1, fac_2, dexp_1, Tp, Qp       
	  ...
	  Tp = 1.001*T
	  Qp = Tp/(theta_rot*2.d0) * sum(   dexp( - EkJ/(ukb*Tp) )   )
	  dQint_dT = (Qp-Q) / (0.001*T)
    ...
    SUBROUTINE compute_frozen_cv_T (T, cv)
      ...
      USE mod_nitrogen_TTLTH_initialize_CFD,     ONLY: theta_vib, cv_tr, cv_rot, Rn2, ek, ekJ, ukb, nb_bins, nb_tvib, nivx

      REAL(KIND=8) :: ev, evp, Tp, cv_vib ! cv_vibl, cv_vibh, den, dexp0, r, , nlim

      REAL(KIND=8), INTENT(IN) :: T
      REAL(KIND=8), DIMENSION(:), INTENT(OUT) :: cv
!	  real(KIND=8), dimension(nlim) :: fvl
!	  real(KIND=8), dimension(nb_bins-nlim) :: fvh	
	  real(KIND=8), dimension(nb_bins)  :: fvl
	  integer	::	i
	  ...
	  do i=1, nb_tvib
			fvl(nivx(i)+1:nivx(i+1))	= dexp( - EkJ(nivx(i)+1:nivx(i+1))/(ukb*T) )/ & 
                                                        & sum(dexp(- EkJ(nivx(i)+1:nivx(i+1))/(ukb*T) )   )
			ev	= sum(fvl(nivx(i)+1:nivx(i+1))*ek(nivx(i)+1:nivx(i+1))) 
			Tp = 1.001*T
			fvl(nivx(i)+1:nivx(i+1))	= dexp( - EkJ(nivx(i)+1:nivx(i+1))/(ukb*Tp) )/ & 
                                                        & sum(   dexp( - EkJ(nivx(i)+1:nivx(i+1))/(ukb*Tp) )   )
			evp	= sum(fvl(nivx(i)+1:nivx(i+1))*ek(nivx(i)+1:nivx(i+1)))
      ! Adding rotational and vibrational contribution for the N2 molecule 
		cv_vib	= (evp-ev)/(0.001*T)
		cv(i+1)	= cv(i+1) + cv_rot + cv_vib	  
	  enddo
    ...
    SUBROUTINE compute_frozen_cv_T_Tv (T, cv)
      ...
      USE mod_nitrogen_TTLTH_initialize_CFD,     ONLY: theta_vib, cv_tr, cv_rot, nb_tvib
    ...
    SUBROUTINE compute_frozen_cv_T_Tr_Tv (T, cv)
      ...
      USE mod_nitrogen_TTLTH_initialize_CFD,     ONLY: theta_vib, cv_tr, cv_rot
    ...
    SUBROUTINE compute_frozen_energy_cv_T (T, cv, e)
      ...
      USE mod_nitrogen_TTLTH_initialize_CFD,     ONLY: theta_vib, cv_tr, cv_rot, Rn2, hf_N, ek, ekJ, ukb, nb_bins 

      REAL(KIND=8) :: cv_vib, den, dexp0, r, Tp, ev, evp 
      ...
      !cv_vib = Rn2*dexp0*(r/den)**2 
		...
		ev	= sum(fv*ek) 
		Tp = 1.001*T
		fv	= dexp( - EkJ/(ukb*Tp) )    /    sum(   dexp( - EkJ/(ukb*Tp) )   )
		evp	= sum(fv*ek)
	  cv_vib = (evp-ev)/(0.001*T)
	  
      ! Adding rotational and vibrational contribution for the N2 molecule 
      cv(2) = cv(2) + cv_rot + cv_vib

      ! Species energy components in thermal equilibrium with translation
      e(1) = cv(1)*T + hf_N
      e(2) = (cv_tr(1) + cv_rot)*T + Rn2*theta_vib/den

    END SUBROUTINE compute_frozen_energy_cv_T 

    !------------------------------------------------------!
    ! This subroutine computes the frozen specific heats at constant volume (thermal nonequilibrium case) and 
    ! the species energy components in thermal equilibrium with translation.
    SUBROUTINE compute_frozen_energy_cv_T_Tv (T, cv, e)

      USE mod_nitrogen_TTLTH_initialize_CFD,     ONLY: theta_vib, cv_tr, cv_rot, hf_N, nb_ns
      ...
      ! Adding rotational contribution for the N2 molecule 
      cv(2:nb_ns) = cv(2:nb_ns) + cv_rot 

      ! Species energy components in thermal equilibrium with translation
      e(1) = cv(1)*T + hf_N
      e(2:nb_ns) = cv(2:nb_ns)*T 
    ...
    SUBROUTINE frozen_cp (T, cp)
      ...
      USE mod_nitrogen_TTLTH_initialize_CFD,          ONLY: Rn, Rn2, nb_ns
	...
	! Non Harmonic oscillator now !!
    SUBROUTINE ev_cvv_ho (Tv, ev, cvv)

      USE mod_nitrogen_TTLTH_initialize_CFD,     ONLY: Rn2, theta_vib, ek, ekJ, ukb, nb_bins

      REAL(KIND=8) :: dexp0, Tr, den, Tvp, evp
     ...
     ! den   = dexp0 - 1.d0
		...
		Tvp = 1.001*Tv
		fv	= dexp( - EkJ/(ukb*Tvp) )    /    sum(   dexp( - EkJ/(ukb*Tvp) )   )
		evp	= sum(fv*ek)
	  cvv = (evp-ev)/(0.001*Tv)
    ...
    SUBROUTINE get_species_energy (temp, e)
      ...
      USE mod_nitrogen_TTLTH_initialize_CFD,    ONLY: theta_vib, hf_n, Rn2, cv_tr, cv_rot, nb_tvib, nb_ns 
implicit none
      REAL(KIND=8) :: T, Tr, er
	  real(kind=8), dimension(nb_tvib)	::	ev, cv
      ...
      call get_energy(temp(posTv:posTv+nb_tvib-1),ev,cv)
	     
      ! Internal energies
      e(1)		 = cv_tr(1)*T + hf_n
      e(2:nb_ns) = cv_tr(2)*T + er + ev(1:nb_tvib)
    ...
    SUBROUTINE get_species_energy_cv (temp, e, cv)
      ...
      USE mod_nitrogen_TTLTH_initialize_CFD,    ONLY: nb_temp, nb_trot, theta_vib, hf_n, Rn2, cv_tr, cv_rot 
      ...
      Tr = temp(posTr)
      Tv = temp(posTv)
  print*, "get_species_energy_cv :: je suis utilise"  
      er      = cv_rot*Tr
    ...
    SUBROUTINE energy (temp, er, ev, e)
      ...
      USE mod_nitrogen_TTLTH_initialize_CFD,    ONLY: hf_n, Rn2, cv_tr, cv_rot, ek, ekj, ukb, nb_tvib, nb_ns
      REAL(KIND=8) :: T, Tr, erot
	  REAL(KIND=8), dimension(nb_tvib) :: cv
      
      REAL(KIND=8), DIMENSION(:), INTENT(IN) :: temp
      REAL(KIND=8), INTENT(OUT) :: er
	  REAL(KIND=8), dimension(nb_tvib), INTENT(OUT) :: ev
	  REAL(KIND=8), INTENT(OUT), DIMENSION(:) :: e
      ...
      !print*, "energy : ", Temp(2:1+nb_tvib),ev
	  !pause
      call get_energy(Temp(2:1+nb_tvib),ev,cv)
	  ! Internal energies
      e(1)			= cv_tr(1)*T + hf_n
      e(2:nb_ns)	= cv_tr(2)*T + er + ev(1:nb_tvib)
    ...
    SUBROUTINE enthalpy (temp, er, ev, h)
      ...
      USE mod_nitrogen_TTLTH_initialize_CFD,    ONLY: Rn, Rn2, nb_tvib
    ...
    SUBROUTINE compute_T (rhoi, rho_eint, temp) 
      ...
      USE mod_nitrogen_TTLTH_initialize_CFD,    ONLY: hf_n, Rn, Rn2, theta_vib
      ...
      REAL(KIND=8) :: rho_N_hfn, rho_N_Rn, rho_N2_Rn2
      REAL(KIND=8) :: T, Told
      REAL(KIND=8) :: res
      ...
      ! Useful data 
      rho_N_hfn  = rhoi(1)*hf_n
      rho_N_Rn   = rhoi(1)*Rn
      rho_N2_Rn2 = rhoi(2)*Rn2
      
      ! Initial temperature guess (vibration is considered half-excited)
      exact = rho_eint(1)
      g   = exact - rho_N_hfn
      fp  = 1.5d0*rho_N_Rn + 3.d0*rho_N2_Rn2
      ...
      DO WHILE (res.GT.tol)
          ...
          fp     = 1.5d0*rho_N_Rn + 2.5d0*rho_N2_Rn2 + rho_N2_Rn2*dexp_T*(theta_vib/(T*den))**2
          ...
          IF (it.GT.it_max) THEN 
             PRINT*
             WRITE(*,'(A)')'solver_fvmcc_f90:excessive number of Newton iterators, in mod_nitrogen_TTLTH_CFD_prop...'
             PRINT*
...
!
!      USE mod_nitrogen_TTLTH_initialize_CFD,    ONLY: theta_vib, Rn2, Rn, hf_n, tb_Tvl, tb_Tvh, table_energy, interpol, nb_temp
!
!      REAL(KIND=8) :: a, b
!      REAL(KIND=8) :: T, Tvl, Tvh, rho_etr, rho_evl, rho_evh
!      REAL(KIND=8) :: rho_n, rho_n2l, rho_n2h, rho_n2 
!
!      REAL(KIND=8), DIMENSION(:), INTENT(IN) :: rhoi, rho_eint
!      REAL(KIND=8), DIMENSION(:), INTENT(OUT) :: temp
!		
!		real(kind=8)	::	evl, evh, cvl, cvh, Tvl_temp, Tvh_temp
!		integer			::	i
!		
!      ! Initialization
!      rho_etr = rho_eint(1)
...
!      rho_n2l = rhoi(2)
!	  rho_n2h = rhoi(3)
!	  rho_n2  = rho_n2l + rho_n2h
!
!      ! Vibrational temperature 
!      !Tv = theta_vib/DLOG(1.d0 + rho_n2*Rn2*theta_vib/rho_ev)
!	 ! print*, "compute_t_tv", rho_evl/rho_n2l, rho_evh/rho_n2h!, tb_Tv(1), tb_Tv(50), table_energy(1,1), nb_temp
!	  call interpol(Tvl, rho_evl/rho_n2l, tb_Tvl, table_energy(:,1))
!	  call interpol(Tvh, rho_evh/rho_n2h, tb_Tvh, table_energy(:,3))
!	  
!			! Méthode de Newton pour améliorer la prédiction des températures internes (1 itération suffit)
!			!do i=1,2	
!				Tvl_temp = Tvl
!				Tvh_temp = Tvh
!				
!				call get_energy(T,Tvl,Tvh,evl,cvl,evh,cvh)
!				
!				Tvl = Tvl_temp - (evl-rho_evl/rho_n2l) / cvl
!				Tvh = Tvh_temp - (evh-rho_evh/rho_n2h) / cvh
!				
!			!	print*, "compute_T_Tv :: ", tvl, Tvl_temp, tvh, Tvh_temp
!			!enddo
!			!stop
!				
!	  
!      ! Translational temperature
!      a = rho_etr - (rho_evl + rho_evh + rho_n*hf_n)
...
!    END SUBROUTINE compute_T_Tv 
    ...
    SUBROUTINE compute_T_Tv (rhoi, rho_eint, temp) 
      ...
      USE mod_nitrogen_TTLTH_initialize_CFD,    ONLY: theta_vib, Rn2, Rn, hf_n, tb_aTv, table_energy, interpol, & 
                                                    & nb_temp, nb_ns, nb_tvib

      REAL(KIND=8) :: a, b, err_rel
      REAL(KIND=8) :: T, rho_etr, aTvi, aTv_temp
	  real(kind=8), dimension(nb_tvib)	::	rho_ev, aTv, ev, acv
      REAL(KIND=8) :: rho_n, rho_n2tot!, rho_n2h, rho_n2
	  REAL(KIND=8),dimension(nb_tvib) :: rho_n2
      ...
      ! Initialization
      rho_etr = rho_eint(1)
      rho_ev  = rho_eint(2:1+nb_tvib)
      rho_n   = rhoi(1)
      rho_n2  = rhoi(2:1+nb_tvib)
	  rho_n2tot  = sum(rho_n2)

      ! Vibrational temperature 
	  aTv = -1.d-4
	  
	  do j=1, nb_tvib
			call interpol(aTv_temp, rho_ev(j)/rho_n2(j), tb_aTv, table_energy(:,j))
		  
	! print*, "compute_t_tv", 	  aTvl, aTvh
			! Méthode de Newton pour améliorer la prédiction des températures internes (1 itération suffit)
			i=1
			err_rel=1.d0
			do while(err_rel > 1.d-6)
				aTv(j) = aTv_temp
				call get_energy(aTv,ev,acv)
				
				aTv_temp = aTv_temp - (ev(j)-rho_ev(j)/rho_n2(j)) / acv(j)
				err_rel  = abs((aTv(j)-aTv_temp)/aTv(j))
				i=i+1
				if(i>5) print*, 'get_energy :: conv lente', i
			enddo
			aTv(j) = aTv_temp
	  enddo		
			!	print*, "compute_T_Tv :: ", tvl, Tvl_temp, tvh, Tvh_temp
...
!print*, "compute_t_tv", 	  aTvl, aTvh
      ...
      ! Translational temperature
      a = rho_etr - (sum(rho_ev) + rho_n*hf_n)
      b = 0.5d0*(3.d0*rho_n*Rn + 5.d0*rho_n2tot*Rn2)

      T = a/b

      ! Temperature vector
      temp(1)     = T
      temp(posTv:posTv+nb_tvib-1)   = aTv
	  !print*, "comp", temp
    END SUBROUTINE compute_T_Tv 
    ...
    SUBROUTINE compute_T_Tr_Tv (rhoi, rho_eint, temp) 
      ...
      USE mod_nitrogen_TTLTH_initialize_CFD,    ONLY: theta_vib, Rn2, Rn, hf_n 
    ...
    SUBROUTINE post_shock_neq (p1, u1, T1, p2, u2, T2, yi, xN)
      ...
      USE mod_nitrogen_TTLTH_initialize_CFD,         ONLY: nb_ns, nb_temp, nb_trot, urg, mm_N, mm_N2, Rn, Rn2, & 
                                                          solver
      ...
      REAL(KIND=8), DIMENSION(nb_ns) :: xi, mm, Ri, cvi, ei
      ...
      IF (nb_temp.EQ.1) THEN
         ...
         WRITE(*,5)solver(1:length),':: Nitrogen TTLTH -> thermal equilibrium post-shock conditions'
         PRINT*
      ...
      ELSE 
         ...
         IF (nb_trot.EQ.0) THEN 
 
            g  = (5.d0*yi(1) + 7.d0*yi(2))/(3.d0*yi(1) + 5.d0*yi(2))
...
15  FORMAT(A,E14.6,A)
	...
	subroutine get_energy(aTv,ev,acv)
			use mod_nitrogen_TTLTH_initialize_CFD,    ONLY: nb_ns, ukb, table_energy, tb_T, tb_aTv, n_T, ekJ, nivx, nb_bins, ek, nb_tvib
implicit none		
		!real(kind=8), intent(in)  :: T
		real(kind=8), dimension(nb_tvib), intent(in)  :: aTv
		real(kind=8), dimension(nb_tvib), intent(out) :: ev, acv
		
		integer :: i, j, k, szfv
		
		real(kind=8), dimension(nb_bins) :: fv, fv_p_dfv!, kd, kd_n2, kb, kb_n2
		real(kind=8)	::	aTv_p_dTv, ev_p_dev

		if( minval(aTv) < tb_aTv(1) .or.  maxval(aTv) >= tb_aTv(n_T)) then
			write(*,*) ':: Temperature out of range,in get_energy. aTvl, aTvh : ', aTv
			write(*,*) ':: Tmin, Tmax : ', tb_aTv(1), tb_aTv(n_T) 
			print*
!			stop
		endif
		
		do i=1, nb_tvib
			szfv = nivx(i+1)-nivx(i)+1
			!if(allocated(fv)) deallocate(fv); if(allocated(fv_p_dfv)) deallocate(fv_p_dfv);
			!allocate(fv(szfv));
			!allocate(fv_p_dfv(szfv));
			
			if(aTv(i)==0.d0) then;	aTv_p_dTv = 1.d-9;		else;		aTv_p_dTv = 1.000001d0*aTv(i);		endif	
			fv		(nivx(i)+1:nivx(i+1))	= dexp(aTv(i)   *EkJ(nivx(i)+1:nivx(i+1))/ukb) / sum(dexp(aTv(i)	*EkJ(nivx(i)+1:nivx(i+1))/ukb))
			fv_p_dfv(nivx(i)+1:nivx(i+1))	= dexp(aTv_p_dTv*EkJ(nivx(i)+1:nivx(i+1))/ukb) / sum(dexp(aTv_p_dTv	*EkJ(nivx(i)+1:nivx(i+1))/ukb))
			
			ev(i)		= sum(fv		(nivx(i)+1:nivx(i+1)) * Ek(nivx(i)+1:nivx(i+1)))
			ev_p_dev	= sum(fv_p_dfv	(nivx(i)+1:nivx(i+1)) * Ek(nivx(i)+1:nivx(i+1)))

			! Cv
			acv(i) = (ev_p_dev - ev(i)) / (aTv_p_dTv - aTv(i))
		enddo
		!deallocate(fv,fv_p_dfv)
				
	end subroutine get_energy
	
  END MODULE mod_nitrogen_TTLTH_CFD_prop
!------------------------------------------------------------------------------!
